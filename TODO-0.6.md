...
**Current blocker / next action:** cancellation/resume is accepted at `4ff042e` with trusted-main CI `34922732310`; keep that closure. PR 47 repair `725062b` passed exact-head CI `34930008573`. The new native probe identified elevated WebView2 argument handling: an application-scoped machine override exposed the installed baseline's CDP page. Verify the reviewed retention/preflight repairs and this guest configuration on their new committed revision, then repeat the actual native installer/preservation sequence. Complete producer/qualification integration independently. Final qualification and promotion remain open. Intensity: ultra for data preservation and release integrity. No tag changes during debugging.
...
- Retention RED/GREEN: `pwsh -NoProfile -File scripts/tests/verify_settings_retention.ps1` first failed with `Verified settings reads were not retained`. The host now copies the actual settings JSON under lock ownership and binds its filename and SHA-256 into the lifecycle document. The real copy/digest checks and lost-lock refusal passed under PowerShell 7 and 5.1. The workflow's retained artifact pattern includes the settings file. Logs: `.hermes-0.6/pr47-retention-{red,green,ps51}.log`.
...
- Scope: the native-runner deferral below was revisited when its new-hypothesis trigger occurred. Debugger discovery now has direct native evidence; the complete installed baseline-to-candidate preservation sequence still needs execution. This probe does not qualify a final candidate. Review also noted pre-existing cache-content and tuning-value assertion limits; these remain evidence-scope limits under the open preservation gate, not newly waived criteria.
