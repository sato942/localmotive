...
**Current blocker / next action:** cancellation/resume is accepted at `4ff042e` with trusted-main CI `34922732310`; keep that closure. Native settings verification stops before baseline seeding because Sandbox exposes no CDP target. Commit the settings/harness repairs, update PR 47 against current main, and require checks on the resulting revision before merge. Continue producer/qualification integration independently; diagnose native debugger access only against a changed hypothesis or configuration. Final qualification and promotion remain open. Intensity: ultra for data preservation and release integrity. No tag changes during debugging.
...
2026-09-15 correction in progress (base `e7feee726d887bfe50ed747019afb4ea3eb22770`; PR 47's existing `pr-check` [34916604366](https://github.com/sato942/localmotive/actions/runs/34916604366) passed only that base, not this working diff):
