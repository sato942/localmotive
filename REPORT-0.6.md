...
**What the freeze-10 repair pass changed (F9-01..F9-06; handoff `LOCALMOTIVE-FREEZE9-REMEDIATION-HANDOFF.md`):** `benchmark_v2` builds its fallible local client before publishing the active benchmark slot, so a client-construction failure (a certificate or API-key reread) can no longer leave the slot occupied when no benchmark started - covered by a behavioural regression with a recorded mutation proof. The packaged cancellation driver attempts the replacement on exactly one surface per scenario (a UI-only click, then a direct-API call in its own run) while the previous request is provably active (`llamacpp:requests_processing=1` immediately before the invocation), feeds the immediate after-sample into the same strictly bracketed coverage classification (after `1` stays active; a count above `1` is overlap and fails; after `0` is a drained interval and null/missing/error is an unavailable observation, both not-exercised with the reason preserved; `mt06_verdicts` 9/9 with RED proof), keeps the original run's record identity and outcome separate from the replacement's, and never credits missing evidence with active coverage. The qualification verifier validates the real packaged-verification producer schema (source revision, artifact name/size/digest against the inventory, per-check status against the aggregate). The package job writes `packaged-verification-<version>.json`, qualification consumes the lifecycle records the same run produced, the qualification step stages the fresh packaged record into the manifest (`--stage-packaged-verification`; `workflow_integration` 10/10), and a controlled package-stage failure names every missing output. The v0.4.1 preservation leg seeds the released settings/profile keys through the baseline build and requires the upgraded build to read the values back; the cache verifier requires the released `schemaVersion` 1 and rejects a non-array `models` field (`preservation_verifier` 8/8). Freeze-10 candidate `e9a36b3aa075d72713d88af4cc74d64d561f12f8` (portable `d6a6ef663c925f9605f06ea2957ef95409dace00d798ebc28e5666249fd59427`, setup `b935033d2ca2dbaf595c0a20e629053f0b4097ebe4b1059c351d140530ab8652`, msi `bc5c6aae332487430c1d7a238b668e3919f1514f713dd0c9d1ab80e248313b9b`): packaged verification 25/25 `overall_status=PASS` `source_dirty=false`, MT-06 75/75 (0 not exercised), installer payload identity PASS on the staged inventory, RT-06 22/22 with the backend scope recorded, witnesses ALL PASS, a11y A11Y_PASS, manifest and promotion rehearsal green with the four lifecycle legs carried through a two-group ledger (freeze-8 and freeze-9 citations, each validated against the record's own staged digests). The native lifecycle portion remains open: Windows Sandbox still cannot boot a VM on this host, so `GH-04.V3` and the unexecuted native part of `G-06.I2` stay environment-blocked and are not claimed as run.
