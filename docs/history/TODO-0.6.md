...
- REMNANTS in I2 (explicit): the Catalog and Profile screens still live inline in `App.tsx`, and `V03EvidencePanel.tsx` still performs its own acquisition (`invoke("load_calibration_records"|"detect_hardware"|"inspect_model_artifact")`) instead of receiving it — the panel separation is a next slice. Candidate-binding note: the G-03 candidate `0322fbd2…`/G-05/G-06 evidence bind to source `065248a1…`; HEAD has moved, so the release candidate must be re-cut and those records re-bound before G-09. The current packaged binary for S-27 evidence is `bc6ee640…`.
