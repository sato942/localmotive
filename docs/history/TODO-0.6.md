...
| R10 | TLS validator accepts marker-wrapped junk (`MIIB` + base64) and the unit test expects success | V06-S-25.I3; audit S-25 | **Open — in progress** | Requires real certificate/key parsing and pair matching with valid fixtures; marker-based tests replaced. |
...
| R15 | Transport hardening: redirect/proxy policy, pre-check serialization allocation, metadata-then-unbounded file read | V06-S-26, MT-06 resource bounds; audit S-26/MT-06 | **Open — in progress** | Bounded body construction + handle-and-limit file reads + explicit redirect policy, each with a mutation-checked test. |
