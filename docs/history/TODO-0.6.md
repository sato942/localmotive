...
- I3: release-record metrics collected from retrieved run records (`gh run list --workflow ci.yml --limit 8`): conclusions/durations (minutes) SUCCESS 8.7, SUCCESS 17.2, SUCCESS 8.8, SUCCESS 8.8, SUCCESS 10.0, SUCCESS 6.0, SUCCESS 7.3, SUCCESS 7.6 for the eight most recent sampled runs — no failures in the sample, one older run in the 10-row window was `cancelled`. Retention caps are the S-16 constants with their tests (`MAX_CALIBRATION_RECORDS`, `MAX_CALIBRATION_RECORD_BYTES`, prune + quarantine coverage).
