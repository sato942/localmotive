...
#### S-27 progress record — slices 1-3a: catalog, runtime and server facades + About screen (I1/I2 remain)
...
- REMNANTS in I1 (explicit): the measurement/tuning command bodies (benchmark, quality, replay, tuning) still live in `lib.rs`; next slice = those, with the coordinator untouched.
