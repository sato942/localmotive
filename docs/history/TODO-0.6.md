...
| R13 | Release evidence gaps: failure-tolerant diagnostics, preflight capture, public JSON trust, tag/publish rechecks, promised vs public file sets, stale job summary text | V06-G-09/G-10 rows | **Open** | Planned with the workflow/doc batch; each sub-item gets its own disposition. |
| R14 | Owner handoff says main remains at `e530371` while also recording the push; ruleset payload lacks the PR rule; tag creation should bind to a reviewed SHA; unknown `isLatest` field; PR campaign cost undescribed; wrong rust-audit trigger claim | V06-G-08.V1, GH-01/02/03 | **Open** | `RELEASE-REVIEW-0.6.md` §2 corrections land with the doc batch; the review file itself stays verbatim. |
