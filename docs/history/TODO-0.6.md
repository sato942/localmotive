...
**Follow-up remediation (2026-09-12, fifth pass; candidate code freeze `85edee6`):** the owner-provided follow-up review `localmotive-0.6-followup-review-db548c8.md` (R01-R15, committed verbatim) was reconciled item by item. Every finding was verified against the tree before any change: R01 (four jobs read non-direct `needs` outputs), R02 (a manual dispatch on a tag could publish with `publish=false`), R03 (cancel lost at result acceptance in client/measurement/tuning), R04 (the active slot released while an abandoned worker still inferred), R05 (a lifecycle PASS with `preservation=missing-files`/`UNVERIFIED`), R06 (candidate fallback to a published release + no inventory byte match), R09 (`StartsWith` version identity + undocumented MSI payload difference), R10 (marker-only TLS validation), R12 (consumer inventory validation gaps), R13 (release-evidence gaps), R14 (owner-handoff errors), R15 (redirect/proxy/unbounded-body/unbounded-read) were all confirmed and fixed with regressions that fail against the old behavior (mutations MUT-R01a/R02/R03a/b-trial/b-warmup/c/R04/R04g/R05b/R10/R12/R15a/b/c caught). R07 replaced marker-fixture preservation with real released-version persistence (v0.5.0-schema mirror with a user override row; v0.4.1 cache record) and the four-leg qualification matrix; R11 rewrote the packaged cancellation driver around owned PIDs, server-accepted requests, persisted terminal records, bounded cleanup and an overlap-free immediate restart; R08 re-cuts the candidate and binds a canonical qualification manifest to it. **Current checkpoint (2026-09-12, fourth pass; candidate code freeze `7c32fa9`; docs/driver tip follows on `main`):** the fourth pass resolved the remaining release-contract and cancellation items. Publication now consumes the retained verified candidate with its original inventory and validates source identity plus every digest/size before promoting those exact bytes (the `--verify` CLI was repaired: its artifact directory resolves from the inventory's own directory, RED reproduced as ENOENT under the exact workflow invocation, GREEN `PASS producer inventory verified: 3 artifacts at source 7c32fa90b9dd04057ba3c089392b0642f3b0545c`, wrong-source refusal exercised, mutation MUT-W3 caught by the new gate pins). Lifecycle evidence binds to the ORIGINAL candidate inventory's full source SHA with the harness revision recorded separately (up40/main/pres041/pres050 PASS with `sourceRevision 7c32fa9…` and distinct `harnessRevision` values; a substituted `LOCALMOTIVE_SOURCE_REVISION` is refused: `MISMATCH_EXIT=1`; fault legs record a null candidate binding and the witnesses all pass). The health-model fetch gained the verifier-profile loopback seam (`rebase_download_url`, loopback-only plain HTTP, production trust checks intact) and RT-04.V2 is sealed packaged 17/17 with a controlled ~4 MB/s delay fixture: a swap attempt inside the pinned window is denied (EBUSY), tampering both verified runtime copies outside the window is refused with zero children, and restoring the exact bytes returns a full PASS (`release-evidence/0.6.0/attestations/rt04v2-delayed-download-verification.log`). MT-06 gained resource-bound proofs beyond caller latency: three slow-body fixture tests (worker exit at the deadline, exact request counts, torn-down connections; mutations MUT-W1/W2 caught) and a packaged six-cycle resource driver, 37/37 checks PASS. Gates on the freeze: `npx tsc --noEmit` 0; Vitest 141; node gates 126; `cargo fmt --check` 0; clippy `-D warnings` 0; `cargo test` 600 passed / 0 failed / 7 ignored; isolated verify_041 re-run on the committed tree 25/25 PASS. Open boxes are the owner-gated GH-01/GH-02/GH-03 rows, GH-06.V3, G-09 rows and G-10.I1; the environment-blocked RT-06.V3, DC-12.V3, MT-07.V2, S-25.I3 and G-05.I3;  Soak record (2026-09-12, before the authorized push): `npm run check` EXIT 0 on the final tree; `cargo test` 600 passed / 0 failed / 7 ignored on re-runs, including three CONCURRENT full suites (600/0 x3) and ten focused `local_client::tests` repetitions (17/17 each). One-off disclosure, kept visible: the first soak attempt reported `599 passed; 1 failed` and the log filter did not capture the failing test's name; fifteen subsequent full or focused runs were clean, leaving it an unnamed watch item. **RESOLVED (fifth pass):** a capturing soak (`.hermes-0.6/soak2/`, full output retained) reproduced the class: `proc::tests::dropping_a_contained_process_terminates_descendants` failed `the descendant fixture did not start` after its 15 s bound - PowerShell cold start (nested, antivirus-scanned) exceeded it under parallel load. Fix, corrected by CI evidence: a 45 s load-aware bound plus a spawned-record diagnostic. The first attempt broke the wait on parent exit alone; the self-hosted CI run for `3b97899` proved that wrong (`parent_exit=Some(0)` at ~10.6 s while the descendant was still booting under load - the parent always exits on its own schedule, so its exit says nothing about the descendant). The corrected fixture has the parent record the spawned child id (`descendant-spawned.txt`); the wait ends early only when the parent exited AND that record is absent. Mutations: MUT-PROC1 (parent never spawns) FAILED in 183 ms (`spawned=false, parent_exit=Some(0)`); MUT-PROC2 (descendant needs ~5 s, parent exits at ~1 s) now PASSES in 7.9 s - the old logic would have ended the wait at the parent's exit. Post-fix load soak (`.hermes-0.6/soak3/` + rerun): three concurrent full cargo suites with zero failure markers and 10/10 focused repetitions green both times.  The FIRST pushed run (`34673683605`) failed on a REAL latent CI flake and no implementation defect: `scripts/tests/http_retry.test.mjs` left the stalled-fetch test pending on `AbortSignal.timeout`'s unref'd default timer, and the runner cancelled it as `cancelledByParent` / "event loop has already resolved". Lasting fix (not a pin): the request deadline is injectable (`fetchRetry(..., { deadlineMs })`, production default unchanged) and the test bounds its own clock at 200 ms — the file now finishes in 0.33 s instead of 20 s. The fix is mutation-pinned by a release gate (dropping the test's `deadlineMs: 200` fails the gate).  The first re-run (`34673889797`) proved one fix was not enough: the injected 200 ms deadline is still an UNREF'D timer, so the promise could lose the event-loop race regardless of duration. The test now also holds the loop with a referenced keeper timer (cleared in `finally`) — the deadline fires at 200 ms, the keeper holds 1 s — and the gate pins both the injectable deadline and the keeper. Pushed as `946ac1c`; its run `34674036480` completed **success** on the self-hosted runner: `check` 7m18s (frontend checks now green, then Rust fmt/clippy/tests), `rust-audit` 24s, `package-smoke` full packaged build green. First pushed runs `34673683605` and `34673889797` remain visible as the reproduce-and-fix record. Housekeeping watch item: `package-smoke` carries a GitHub Node-20-deprecation annotation for the SHA-pinned `actions/setup-node`.
...
## Follow-up review reconciliation (reviewed revision `db548c820cbdf2d8b289f797ce27d31f1ff338f3`)

**State at reconciliation start (2026-09-12):** branch `main`; HEAD = `origin/main` = `db548c820cbdf2d8b289f797ce27d31f1ff338f3` (exactly the reviewed revision; no later commits existed). **Campaign-6 checkpoint (later the same day):** every R01-R15 item was implemented with RED-first regression evidence and mutation checks; the candidate was re-cut at freeze `85edee63d060c43d05df3770d168a81b7421d6df` (portable `095b153663aaef71f91a0dc7f67c5ac119c523b81521b00c82d67d32cb5f02ed`, setup `533b669c3cde3dfdea14865105ff3212b2390d8a7b2527ec6f8af85689ae933c`, msi `9dac49aa3e66c15fb592ccd613885b5088a4838e0b4328b301dfa46e39d899bc`; inventory PASS, source 85edee6) because R01-R05 and R10-R15 changed shipped code. Per-campaign evidence: MT-06 cancellation 33/33 checks (`llamacpp:tokens_predicted_total`-gated cancels, persisted `terminalOutcome=cancelled` per cycle, owned-PID tracking across the deliberate restart, immediate-restart overlap-free); DC-04 command path 13/13; churn clean; supervision/tamper/FE-16/FE-05.V3/vitems PASS; packaged verification 25/25 `overall_status=PASS` `source_dirty=false` at `928865e`; RT-04.V2 17/17; the four-leg lifecycle matrix PASSES with the R07 flavors (upgrade v0.4.0 + v0.5.0 and preservation v0.4.1 + v0.5.0, every leg `status=PASS` with `preservation=PASS`, all bound to source 85edee6 via the inventory; one preservation leg hung in the sandbox MSI step on its first attempt, was killed, and passed on retry with unchanged candidate bytes); evidence witnesses 7/7 (`ALL WITNESS LEGS PASS`: missing-assets, timeout, malformed-result, cancellation, preservation-missing, plus the unexpected-failure leg, plus live-lock refusal and stale-lock takeover - the last two harden an incident found during this campaign: a killed chain left an orphaned attempt running and its late TIMEOUT record overwrote a newer clean PASS for the same scenario; the harness now holds a per-scenario lock, live owners make a second run refuse, dead owners are taken over, and every evidence write re-authorizes against the lock - RED/GREEN proven with the pre-lock harness racing the lock and the patched harness refusing it; the two preservation records were restored to the provenance-clean retry outputs from the committed revision); packaged health 7/7; MT-01d decode 867.25 tok/s (p95 878.93, n=5, warm); negative controls: the historical doctored installer fails the sandbox with identity (`expected 0.6.0 exactly but found '0.5.0'`), and a byte-doctored setup against the freeze-6 inventory is refused at `resolve-installers` naming both digests (`ecca1fef...` vs `533b669c...; refusing to qualify unverified bytes`). The canonical qualification manifest (committed at `release-evidence/0.6.0/qualification-manifest-0.6.0.json`; scratch copy under `.hermes-0.6/`) binds this source revision, the three artifact digests, and every record (packaged verification 25/25 PASS, lifecycle x4, witnesses, MT-06 33/33, DC-04, RT-04.V2, negative control); records from earlier candidate cuts remain history, labelled superseded, never rewritten. Working tree clean except the untracked review record `docs/localmotive-0.6-followup-review-db548c8.md`, which is preserved verbatim in this repository. Authorizations unchanged: local builds/tests/verification and `main` pushes to the self-hosted runner are authorized; tag creation, public publication, ruleset application, and hosted-runner PR campaigns remain owner-gated (G-08/G-09 rows below). The review file names R01–R15; every finding below is verified against the tree before any change, and changes are RED-first with an explicit mutation where a guard exists.

| R | Reviewed claim (short) | Maps to | Status | Evidence |
|---|---|---|---|---|
| R01 | `release.yml` jobs read `needs.*` outputs of non-direct dependencies; the quality job would fail its own SHA assertion at `release.yml:130` | V06-GH-01/GH-02 publish path; audit GH-01/GH-02 | **Confirmed and fixed** (four jobs). Implementation `5c31c70`. Blocker: none. | RED: `verify_workflow_gates` direct-dependency rule fails on `[rust-audit, quality, package, clean-account-lifecycle]` (`.hermes-0.6/r01-red-list.txt`). Fix: `quality: [resolve, rust-audit]`, `package: [resolve, rust-audit, quality]`, `clean-account-lifecycle: [resolve, quality, package]`, `publish: [+resolve]`; the checker now applies the direct-dependency rule to every workflow. Mutation MUT-R01a (revert `quality`) caught. |
| R02 | Manual dispatch at a tag ref with `publish=false` passes the publication predicate; gates only substring-match the `if:` expression | V06-G-09.I1; audit G-09 | **Confirmed and fixed + semantic test**. Implementation `5c31c70`. Blocker: none. | RED: the new table-driven predicate evaluation fails `dispatch/tag/publish=false` (126/1). Fix: the tag branch requires `github.event_name == 'push' && startsWith(github.ref, 'refs/tags/v') && needs.quality.outputs.tag == github.ref_name`. The gate extracts the predicate from `release.yml` and evaluates it over push/manual × branch/tag × matching/mismatching tag × publish on/off. Mutation MUT-R02 (drop the event binding) caught. |
| R03 | A cancel set before response acceptance is lost: client returns the result; measurement records the last trial as Succeeded; the tuning finalist can be confirmed | V06-MT-06 (cancellation contract); audit MT-06 | **Confirmed and fixed at all three layers**. Implementation `0c62d54`. Blocker: none. | RED first: client `Ok((200, …))` returned after cancel; measurement recorded Succeeded for the final trial; tuning confirmed the winner. Fix: acceptance-time flag check in `post_json_cancellable`; acceptance-time arms in the warmup AND trial loops (`Cancelled` terminal outcome, timings discarded); the finalist confirmation re-checks cancellation and withholds the winner. Mutations MUT-R03a (client), MUT-R03b-trial, MUT-R03b-warmup, MUT-R03c (tune) all caught. Mutation-check note: cargo keeps a stale binary if a restored file keeps an old mtime — restores now touch the file. |
| R04 | `benchmark_v2` clears its active slot while an abandoned worker may still infer; the worker counter is test-only and excludes nothing | V06-MT-06.V2/MT-05; audit MT-06/MT-05 I2 | **Confirmed and fixed**. Implementation `a3ba1e7`. Blocker: none. | The whole benchmark run now shares ONE client instance; the run waits (`wait_for_worker_drain`, 50 ms poll, 300 s ceiling above every request deadline) for all cancellable workers to exit BEFORE it finalizes the manifest; expiry discards the record. The accessor is production code. RED-first test `r04_worker_drain_waits_for_the_abandoned_slow_request_to_exit` (slow-body fixture; asserts the worker is still alive after the cancel and that the drain actually waited). Mutations MUT-R04 (drain returns immediately) and MUT-R04g (drain removed from the run path, caught by the new release gate) both caught. |
| R05 | Lifecycle PASS with `preservation=missing-files`/`UNVERIFIED` (only literal `FAIL` threw) | V06-GH-04.I3; audit GH-04/G-06.I2 | **Confirmed and fixed**. Implementation `ba1e450` (+ witness regeneration `f1ec328`). Blocker: none. | The evidence document flips to `status=FAIL` at stage `preservation-verification` and the run throws; no result short of a verified PASS exits zero. New witness leg 5 (`witness-preservation-missing`, `-FaultSimulation preservation-missing`) proves why: FAIL document at the right stage with `preservation.status=missing-files` AND a nonzero exit. MUT-R05b (pre-fix false-green behavior) caught by the leg's nonzero assertion. All witness legs regenerated after the fix (now 7/7 including the live-lock refusal and stale-lock takeover legs; harness revision recorded separately). |
| R06 | The lifecycle job could silently fall back to the published release when `-CandidateDir` names missing files, and never checked its bytes against the run's inventory | V06-GH-03.I2, GH-04.V3; audit GH-03/GH-04 | **Confirmed and fixed**. Implementation `ba1e450`. Blocker: none. | An explicitly supplied `-CandidateDir` is a hard contract: missing directory/files refuse (no fallback), and both installers must match the inventory's `sha256` and `sizeBytes` before any sandbox work; the evidence carries `candidateInventorySha256` on every outcome. Negative controls: a single flipped byte in a staged setup was refused before the sandbox (`64fc16f8…` vs inventory `e405cd1e…`, freeze-5 era), and at the freeze-6 campaign a byte-doctored setup against the freeze-6 inventory was refused at `resolve-installers` naming both digests (`ecca1fef…` vs `533b669c…; refusing to qualify unverified bytes`). |
| R07 | Preservation canaries are generic fixtures, not real v0.4.1/v0.5.0 persisted data; release.yml requests only v0.4.0 | V06-G-06.I2 (reopened below), GH-04.I3 | **Confirmed and fixed**. Implementation `1030484` (+ witness rebinding `04bb81d`). Blocker: none. | Fixtures are now derived from the released sources: `scripts/sandbox/build_preservation_fixture.py` rebuilds `canary-mirror.sqlite.b64` from the v0.5.0 `catalog_db.rs` DDL (one verified row + one user-override row with `user_sourced=1` and sentinel values) and emits `canary-catalog-cache.json`, the v0.4.1 `CacheRecord` format that version really persisted. `verify_preservation.py` now asserts recovery, not a marker: schema version, the user override model AND file rows with their ownership flags, and (cache flavor) a parseable cache record; six control cases pass/refuse as expected (valid PASS; deleted override row FAIL; ownership flipped FAIL; tampered canary FAIL; cache valid PASS; cache missing FAIL). Both harnesses carry a `mirror|cache` flavor; `release.yml` runs the explicit four-leg matrix (upgrade v0.4.0/v0.5.0, preservation v0.4.1/v0.5.0) with the correct persistence per baseline, and the stale “non-gating policy” summary is replaced by the real policy (publication depends on the job). The refreshed matrix ran in the freeze-6 campaign: all four legs PASS with `preservation=PASS` bound to source `85edee6` via the inventory (evidence files committed; one preservation leg hung in the sandbox on its first attempt, was killed, and passed on retry). |
| R08 | Committed qualification evidence mixes candidate identities (inventory at `3a2b06e`, packaged at an earlier portable, lifecycle at `7c32fa9`); `verify_041` defaults `source_revision` to HEAD | V06-G-03/G-04/G-08; audit G-03/G-04 | **Resolved**. Implementation: freeze `85edee6` + campaign evidence `1c0ee34`. Blocker: none. | The candidate was re-cut at `85edee6`; the campaign regenerated the inventory and every qualification record at one source revision and committed the canonical manifest (`release-evidence/0.6.0/qualification-manifest-0.6.0.json`) binding source + artifact digests + every record; older records remain history labeled superseded, never rewritten. `verify_041` received the explicit frozen revision (`source_dirty=false`, 25/25 PASS). |
| R09 | MSI-installed payload hash differs from NSIS without a documented expectation; version identity uses `StartsWith` | V06-GH-04.I3 (reopened upgrade criterion), G-08 register | **Confirmed and fixed (identity part); payload expectation now explicit**. Implementation `a1b06e1`. Blocker: none. | `Assert-AppVersion` compares exactly (4-part canonical form normalizes; `0.6.00`-style near misses fail). The result document now carries `nsisPayloadDigest`, `msiPayloadDigest`, and an `installedPayloadNote` stating the established expectations (one byte-identical NSIS payload across fresh/update/preservation — enforced; MSI measured and version-verified on its own path; cross-bundler byte identity not claimed; no sidecar claim beyond the executable). Gate added. Fresh lifecycle evidence was regenerated in the freeze-6 campaign (four legs; each result document carries the payload digests and notes). |
| R10 | TLS validator accepts marker-wrapped junk (`MIIB` + base64) and the unit test expects success | V06-S-25.I3; audit S-25 | **Confirmed and fixed**. Implementation `ac3a419` (+ register `822c068`). Blocker: none. | The validator decodes strict PEM (`rustls-pemfile`), parses the certificate as real X.509 (`webpki::EndEntityCert`), structurally extracts the certificate SPKI, derives the key SPKI from PKCS#8/PKCS#1/SEC1 DER (RSA and EC), and requires them equal. Marker-junk fixtures in `core.rs` now expect the parse failure (one previously expected success). New test covers: real RSA pair PASS, junk cert FAIL, junk key FAIL, unrelated RSA pair FAIL (`do not belong together`), real EC P-256 PKCS#8 pair PASS, cross-family mismatch FAIL. Mutation MUT-R10 (pair comparison disabled) caught. Two dependency additions are direct re-exports of crates already in the tree plus the small `rustls-pemfile` PEM decoder. |
| R11 | Packaged cancellation driver measures process names rather than owned PIDs, treats enumeration errors as zero, omits the run record/terminal outcome and restart-overlap scenario, and its log is not in the committed evidence | V06-MT-06.V3/G-05; audit MT-06/G-05 | **Implemented and verified at the freeze**. Implementation `85edee6`. Blocker: none. | `g05_mt06_cycles.mjs` was rewritten: owned processors are parent-scoped to the app PID via `Win32_Process` (`ownedServerPids`, enumeration failure THROWS - the old `catch { return 0; }` is gone), each cancel is gated on the server actually reporting the request in flight (`llamacpp:requests_processing == 1` observed first), the persisted benchmark manifest must carry `terminalOutcome == "cancelled"`, cleanup latency is bounded and recorded per cycle, the final cycle exercises an immediate restart and fails on any observed overlap (two concurrent requests or two servers), and every run writes a digest/source-bound `localmotive.mt06-cancellation.v1` result file (`MT06_EVIDENCE_PATH`). The packaged run completed at the freeze: 33/33 checks, four cycles each accepted a server-reported request before cancelling, every cycle persisted `terminalOutcome=cancelled`, owned children tracked across the deliberate stop/restart, the immediate restart showed no overlap, and the digest/source-bound result file is committed. |
| R12 | Consumer inventory validation lacks schema/version/exact-set/duplicate checks | V06-G-02/G-04 publication gate; audit G-02/G-04 | **Confirmed and fixed**. Implementation `a1b06e1`. Blocker: none. | `verifyPublishedInventory` fails closed on schema ≠ 1, non-semver or unexpected release, empty/duplicate/renamed/short artifact sets, plus the existing size/digest/checksum/identity checks; the workflow passes the expected release; the eight-case behavioral test exercises the real verifier; MUT-R12 (drop the exact-set rule) caught. |
| R13 | Release evidence gaps: failure-tolerant diagnostics, preflight capture, public JSON trust, tag/publish rechecks, promised vs public file sets, stale job summary text | V06-G-09/G-10 rows | **Confirmed and fixed**. Implementation `606656b`. Blocker: none (the peeled-tag/source comparison is implemented and tested without publishing). | (a) the lifecycle evidence uploads for every outcome (`if: always()`), and a failed publish read-back retains `public-readback/` + `release-notes.md`; (b) the harness preflight binding moved inside the try - proven: a source-revision mismatch now writes `FAIL` at stage `initialization` (probe run `04bb81d5…` mismatch, evidence retained); (c) the public inventory must be byte-identical to the producer copy (`cmp`) AND re-verifies with the inventory verifier against the public set; (d) the remote tag is re-resolved (peeled) immediately before publication and drift refuses; (e) the review's asset sentence now separates the six release assets from the workflow artifacts; (f) the stale "non-gating policy" lifecycle summary was replaced with the real policy (publication depends on the job). |
| R14 | Owner handoff says main remains at `e530371` while also recording the push; ruleset payload lacks the PR rule; tag creation should bind to a reviewed SHA; unknown `isLatest` field; PR campaign cost undescribed; wrong rust-audit trigger claim | V06-G-08.V1, GH-01/02/03 | **Confirmed and fixed**. Implementation `606656b`. Blocker: the owner must apply the ruleset payloads and run the three-run PR campaign before tagging (owner-gated, prepared in RELEASE-REVIEW-0.6.md). | The bootstrap intro now states the current remote state (`e530371 -> 9b04b47 -> db548c8` + the remediation series) and scopes trusted jobs (`check`/`rust-audit`/`package-smoke`) to main pushes only, with `pr-check` as the sole PR path. The main ruleset payload now includes the `pull_request` rule (required PRs) alongside the `pr-check` status, no-force-push and no-deletion rules. Tag creation binds to the reviewed SHA from the evidence manifest (never HEAD), run watching binds to that commit, and the Latest readback uses the supported releases API instead of the unsupported `isLatest` field. The PR campaign is stated as three hosted runs with its authorization requirement. The review file `localmotive-0.6-followup-review-db548c8.md` itself stays verbatim. |
| R15 | Transport hardening: redirect/proxy policy, pre-check serialization allocation, metadata-then-unbounded file read | V06-S-26, MT-06 resource bounds; audit S-26/MT-06 | **Confirmed and fixed**. Implementation `57a07c9` (+ clippy `f6a9105`). Blocker: none. | `redirect::Policy::none()` + `no_proxy()` on the builder; request bodies serialize through a `BoundedVec` writer that refuses bytes past `MAX_LOCAL_REQUEST_BYTES` (the old `to_vec` pre-check allocated the whole body first); `read_bounded_file` now reads via the handle with `take(limit + 1)` and refuses overflow, so a growing or lying file can never copy unbounded data. Tests: redirect-hop refusal (target port never receives the request), bounded-writer refusal, over-limit file refusal. Mutations MUT-R15a (follow redirects), MUT-R15b (unbounded writer), MUT-R15c (unbounded read) all caught. |

**Reopened completion claims.** R05/R06 reopen the lifecycle acceptance language: a preservation step that never ran and an unverified candidate binding were compatible with a PASS verdict. R09 reopens the GH-04 upgrade criterion (exact installed version identity) and narrows the payload claim to what is measured. R04 reopens MT-06.V2's ownership statement (the test-only counter did not exclude replacement work). R01/R02 reopen the publication path itself (a manual dispatch on a tag could publish with `publish=false`, and four jobs could never pass on a real tag). R07/G-06.I2 and R08/G-03/G-04 reopen the qualified-candidate record: all qualification evidence must bind to ONE source revision and ONE artifact set, and the preservation matrix must exercise real released-version persistence. These reopenings are recorded here; the original evidence records stay in place and are labeled superseded rather than rewritten when the campaign regenerates them.

**Disproved claims (kept for the record).** None of R01–R15 was disproved; each investigated claim was reproduced against the tree before its fix. The review's negative observations that already had visible dispositions (RT-04.V2 store repair, the one-off `599/1` soak result, the Node-20 annotation) remain as disclosed watch items, not new findings.
...
**Freeze-9 R16 remediation (2026-09-13, ninth pass; candidate code freeze `eb01bc9f9741`):** the owner-directed R16 goal was reconciled outcome by outcome against the tree and every independently executable item was fixed, re-verified and re-qualified on new bytes. **Cancellation and ownership:** the benchmark drain bound now derives from the workload's own request deadline plus teardown slack (the removed fixed ceiling sat below the default deadline, so ownership could outlive an abandoned request), one client per run covers prompt preparation as well as the measured attempts, and the command holds its slot and reservation until the drain returns on every exit path - prep cancel, ordinary error, drain expiry, cleanup failure, discarded result; regressions `r16_the_drain_bound_follows_the_workload_request_deadline`, `r16_the_benchmark_command_releases_ownership_only_after_its_workers_exit` and the gate pins in `release-gates.test.mjs` (with the fixed-ceiling ban) fail against the old behavior. **Immediate restart:** the packaged driver attempts the replacement at the earliest UI-actionable moment after the cancel is accepted - before any settle or drain wait - capturing the server metric at the cancel instant, at attempt entry and at the click; the refusal is proven by the API verdict/UI verdict and no overlap was observed across the six-cycle run (59/59 checks PASS, `.hermes-0.6/final-candidates-5/attestations/mt06-cycles-result.json`). **TLS and local transport:** the raw SEC1 branch passed the private scalar instead of the complete structure to the SPKI extractor, so every complete `EC PRIVATE KEY` file failed as malformed; the parser now hands the complete SEC1 DER to the extractor, the fixture reproduces `openssl pkey -traditional` bytes exactly, and truncated structures still fail closed (`r16_a_raw_sec1_private_key_is_parsed_and_matched`, mutation caught). **Lifecycle and migration:** the harness now takes an OS-exclusive lock, asserts ownership before every evidence write, publishes by atomic rename, and refuses while another live owner holds the scenario; the witness harness drives a real second process for the live-lock refusal and the stale-owner takeover, and the freeze-9 matrix produced the v0.5.0 upgrade and v0.4.1 preservation legs natively on the freeze-9 bytes (both PASS, including their preservation checks) with the v0.4.0 upgrade and v0.5.0 preservation legs carried forward through the ledger; every witness leg passed. **Candidate and evidence identity:** `scripts/build_qualification_manifest.mjs` generates the canonical manifest, `scripts/verify_qualification_manifest.mjs` validates the inventory reference, every mandatory record, required outcomes, staged payload digests and source/digest cross-relationships, and `scripts/verify_installer_payloads.mjs` records the installed NSIS/MSI payload identities with a CDP functional probe of those exact bytes; the fourteen negative controls in `scripts/tests/qualification_manifest.test.mjs` plus `scripts/tests/release_promotion.test.mjs` refuse source-only, digest-only, missing-record, wrong-outcome, path-escape, changed-byte, missing-asset and moved-tag cases. **Release behavior:** the tag-push workflow verifies without publishing (no publish job, no write permission, publication mechanisms forbidden by the gate policy) and `.github/workflows/release-promote.yml` publishes only for the owner with the exact `PUBLISH <tag>` phrase, a green `Release verify` run for the same commit, required checks green and `dry_run=false`, consuming the exact qualified bundle without rebuilding or rewriting checksums. **Installed payloads:** report the NSIS payload `7ec790cf67cc70e3051621b6ef36dc494a16f39d8cd35db495dcdb19a4b87e29` and the MSI payload `3a736bb9ee2928f02d73e718d51092cdc126e7920c22392eff33f909b9aa4f08`; both were launched from their extracted bytes and evaluated over CDP (8 navigation buttons, managed controls, rendered body), and the in-sandbox legs record the installed digest of the same bytes. **Trace to the follow-up review register:** cancellation and ownership R03/R04; immediate restart R11; TLS and local transport R10 (with the preserved bounded-read and bounded-response protections of R15); lifecycle preservation, staged-copy comparison and migration coverage R05/R06/R07; mixed candidate identities and consumer inventory validation R08/R12; release dependencies, publication authorization, retention/readback and the owner handoff R01/R02/R13/R14; installed payload identities R09.

**Honest coverage:** RT-06 records four installed backends, three vendor-refusals, per-backend describe+launch verdicts and the verification root each relied on; the seven-installed-backend criterion remains open on this single-vendor host, and MT-07 keeps its deterministic-snapshot credit with the physical-hardware portability gap named as a hardware-program residual. Gates on the freeze: `npx tsc --noEmit` 0; Vitest 141/141; the new node suites (qualification manifest 14/14, release promotion 9/9, installer payloads 5/5) plus the existing ones green; `cargo fmt --check` 0; clippy `-D warnings` 0; `cargo test` 613 passed / 0 failed / 7 ignored. Evidence: `release-evidence/0.6.0/qualification-manifest-0.6.0.json` (source `eb01bc9f97417e74ce7de7da85c86582f42ceb67`), the staged inventory, and the attestations under `release-evidence/0.6.0/attestations/`.
...
- [x] **V06-RT-06.V3** — Measure bytes hashed, job counts, and elapsed time for listing, selecting, and launching on a representative supported Windows host with all seven backends installed; record the environment and results. Run on this host (Ryzen 9 9950X3D + RTX 5090, adapter `luid:0000000000014f4f`, catalog `b10816`): the catalog exposes all seven Windows x64 backends; four are eligible on a single-vendor host and were exercised (cpu reused 169-181 ms; cuda-12.4 reused 717-752 ms; cuda-13.3 reused 506-527 ms; vulkan freshly installed 2,916 ms at 35,228,033 bytes); the three non-NVIDIA backends are refused by the runtime-compatibility guard with named reasons (`GPU adapter luid:... (nvidia) is incompatible with the {openvino,rocm,sycl} runtime`; with no adapter, `The selected runtime requires one bounded GPU adapter ID`) - a seven-simultaneous-backend host would need three GPU vendors, so the maximum eligible set was qualified and the refusals proven. Listing: 4 records, second pass hashed 0 bytes (the verified-install lease/cache works); selecting: `describe_runtime` 379-388 ms with `managedVerified=true`; launching: managed health run PASS in 3,336-3,438 ms; instrumentation counters (jobs started/coalesced/bytes hashed/cancelled) captured around every step. Driver `scripts/g05_rt06_all_backends.mjs` 12/12 checks PASS on the packaged binary; evidence `release-evidence/0.6.0/attestations/rt06-all-backends.json` + `rt06-full-run.log`. **Trace:** [Audit RT-06](./localmotive-comprehensive-audit.md#rt-06).
...
- [x] **V06-DC-01.V3** — Run the packaged Windows sequence: successful refresh, close, restart within 26 hours, open HF Catalog, and browse/filter a cached entry; record the displayed cooldown and request behavior. **Trace:** [Audit DC-01](./localmotive-comprehensive-audit.md#dc-01).
...
- [x] **V06-DC-04.V2** — Against a controlled server, exercise an approved override with correct and incorrect SHA-256, then remove it and verify subsequent download authorization fails. **Trace:** [Audit DC-04](./localmotive-comprehensive-audit.md#dc-04). **Closed (2026-09-12):** supported-command-path run 13/13 against controlled loopback bytes (correct digest publishes, wrong digest refuses and publishes nothing, removal revokes with no network touch); evidence `release-evidence/0.6.0/attestations/dc04-v2-command-path-verification.log`; record below.
...
**Finding-level hold (2026-09-12, per owner directive):** cells V1-V3 are verified with packaged evidence (fe16 walk, fe05v3 two-profile walk, vitems_d F52/F51 legs), but the FE-05 finding itself is kept OPEN until its completion criteria are explicitly signed off with the release decision; the owner directed that a finding is not closed by its cells alone. Completion criteria for that sign-off: (1) run state and evidence survive navigation and panel teardown (V1); (2) a run completing while the user is elsewhere is retained and visible (V2); (3) saved manifests and anchors recover both profiles' provenance by identity (V3); (4) remedy items I1-I5 each map to their implementing commit and a regression test.  
...
**Owner-gate recheck (2026-09-12 third pass):** `gh api repos/sato942/localmotive/rulesets` -> `[]`; `gh api repos/sato942/localmotive/branches/main --jq .protected` -> `false`; `git ls-remote --tags origin` lists `v0.4.0`, `v0.4.1`, `v0.5.0` only. The four open rows stay unverified until the owner applies the rulesets and a real PR exercises the checks. Completion criteria: a main-branch ruleset requiring the `pr-check` check with no bypass actors for ordinary contributions and force-push/deletion forbidden; a benign PR showing the expected checks with stable names; a controlled failing check blocking the merge; a recorded readback of the effective ruleset including bypass actors.  
...
**Status:** Complete (slices 1-3g: five Rust facades + all eight view screens + evidence-adapter seam; see the S-27 record) · **Priority:** Unscored audit recommendation · **Owner:** Unassigned  
...
- [x] **V06-S-27.I1** — Split catalog store/cache, supervised job lifecycle and runtime authorization/installer facades behind tested contracts, keeping Rust authoritative. **Trace:** [Maintainability assessment](./localmotive-comprehensive-audit.md#maintainability-assessment).
...
**Status:** Partially verified; managed install/tamper/NTFS/SQLite/cancellation/Job-Object/TLS-auth/warm-benchmark cells observed on the packaged binary; hard-link, partial-byte retention, orphan-repro, screen-reader/NVDA, high-contrast-mode and live-cloud cells remain OPEN · **Priority:** Release/verification gate derived from audit · **Owner:** Unassigned  
...
#### G-05 packaged Windows wave — fourth batch: TLS/auth profiles, certificate-error fix, download cancel and stop supervision (source `69e6ff0`, candidate sha256 `3663c52f7e455a91f47a204336e886e8f78e9b91d8fce683d22243f6ff998917`, 20 963 840 bytes)

- **TLS/auth local profile (G-05.I2) VERIFIED.** The profile editor drove `--api-key-file`, `--ssl-key-file`, `--ssl-cert-file` into a real launch against the approved CUDA runtime: the server logged `listening on https://127.0.0.1:8080`; `/props` answered 401 without the key and 200 with `Authorization: Bearer <key>`; `/health` stayed public. The app reached the live state and its own benchmark client ran over TLS: mean **1004.76 tok/s**, median 1006.10 (R1 999.09, R2 1006.10, R3 1009.08). Stable mapping on identical GPUs stays untested (single GPU in this host, per the item's fallback: the row remains a visible limitation).
- **Defect found and fixed (packaged):** a certificate pair that `openssl req -x509` produces by default is marked `CA:TRUE`; webpki refuses it as the server certificate (`CaUsedAsEndEntity`). The startup health wait discarded the client error and kept retrying until its 600 s timeout, so the launch neither succeeded nor said why. The wait now returns an immediate `health_tls` verdict carrying the raw verification error plus the recovery step (regenerate with a subjectAltName and `basicConstraints=critical,CA:FALSE`), and a timeout now names the last transport error. Red-before-green: the regression failed on the old code with `phase health_timeout` after the full 30 s; it passes in 0.02 s after the fix. Mutations M1 (fail-fast removed), M2 (guidance stripped) and M3 (transport recording removed) are all caught. Packaged confirmation on the rebuilt exe: the CA:TRUE pair produced the guidance message instantly (verbatim: "invalid peer certificate: Other(OtherError(CaUsedAsEndEntity))) The SSL certificate that llama-server presents is marked as a CA (basicConstraints CA:TRUE). llama-server must present an end-entity certificate. Regenerate the pair with a subjectAltName and -addext basicConstraints=critical,CA:FALSE, then select both files again."); the corrected pair (SAN + CA:FALSE) reached live and benchmarked. No `llama-server` process remains after app exit (kill-on-close job).
- **Download cancellation (G-05.I1 partial):** the catalog download control `Keep & stop` appeared in flight and accepted a click; the notice read "Download cancelled while checking the remote file"; the per-job lock file existed during flight and was removed on cancel. Partial-byte ("keep partial") retention was NOT exercised at the packaged layer because the cancel landed in the remote-check phase - remains open.
- **Stop supervision:** clean start/stop cycles terminate the child within 1 s (two independent observations, probe self-checked against `tasklist`). One earlier observation during a mixed sequence (cancel clicks during starts, then restarts) left a `llama-server` answering `/health` after "Stop server" while the app showed idle; the child was inside the app's kill-on-close job and died with the app. Scripted churn (three rounds of start/cancel/start/stop) did not reproduce it. OPEN observation - repro outline: Start, click Cancel within the starting phase, Start again, wait live, Stop, then check for a surviving `llama-server`.
- **Housekeeping recorded:** the S-17 guard's acquisition allowlist now includes `src/evidence-adapter.ts` (writers in ranking/measurement/sharing are still asserted clean); the branding scanner skips the git-ignored `.hermes-0.6` scratch dir and allowlists the G-05 driver's legacy-name comment.
- Gates: `cargo fmt --check` PASS, clippy 0, `cargo test --lib` 583 passed / 0 failed / 4 ignored, `npm test` 145/145, `npm run check` exit 0 (branding PASS, build 357.13 kB). Evidence: `.hermes-0.6/g05-tls.log`, `g05-tls-retry.log`, `g05-tls-live.log`, `g05-tls-fix.log`, `g05-download-cancel.log`, `g05-stop-supervision3.log`, `g05-churn.log`, `g05-lock.log`, `g05-dc01.log`, `g05-tls-npm-*.log`.
- Residual cells for G-05: hard-link case, partial-byte download retention, the orphan-supervision reproducer (I1); Narrator/NVDA and high-contrast mode, live cloud credential scenarios (I3, authorized accounts not available). All remain visibly unchecked.

#### G-05 packaged Windows wave — first batch (candidate `f556fba6…`, source `e8a42ca`)

Environment: Windows 11 (26100), Ryzen 9 9950X3D + RTX 5090 (driver 610.74), packaged portable exe built from `e8a42ca`. All runs used the real WebView2 window over CDP on this host.

- Real managed install through the UI (RT-01/RT-07-adjacent): official `llama-b10816-bin-win-cuda-13.3-x64.zip` + `cudart` downloaded and installed into the primary root `Localmotive\runtimes\b10816\cuda-13.3` (672 MB on disk, `runtime.json` with `releaseTag b10816`, `releaseCommit 427291b5…`, `contentManifestSha256 bf60d7f3…`). Evidence: `.hermes-0.6/g05-install.log`, `.hermes-0.6/g05-managed-install2.log` (earlier driver attempts died on the known non-tty stdin quirk and are recorded, not hidden). Adapter-compatibility guard observed refusing OpenVINO on this NVIDIA machine ("GPU adapter luid:000000000000014f4f (nvidia) is incompatible with the openvino runtime").
- Legacy re-use (RT-01): before parking, the app adopted `GGUF Pilot\runtimes\b10816\cpu\llama-server.exe` as active ("Your existing executable matches this package"), with the path input showing the legacy path. The legacy tree was parked with a reversible rename to make CUDA installable, then restored byte-for-byte (`UNPARKED`).
- Seven-stage health baseline (RT-02.V3, real process): "NVIDIA CUDA 13.3 passed all seven managed-runtime health stages", including "Bounded device enumeration identified the approved backend and exact adapter" and "The approved backend completed a bounded operation on the exact adapter", model SHA-256 e3131339… pinned SmolLM2.
- RT-04 tamper (packaged, inert marker): the managed `llama-server.exe` was replaced by a 4 096-byte marker exe whose execution writes `MARKER_RAN.txt`. Re-running health produced `trust_failure`: "Managed runtime cuda-13.3 is not installed or fails content verification. Install it from the Runtime screen." with later stages SKIPPED; `MARKER NOT EXECUTED`; no `llama-server` process appeared. Restoring the original bytes (sha256 `907a7ac7…`) returned "passed all seven managed-runtime health stages" immediately — no abandoned locks. RT-04.V1 checked (rejection before an inert marker runs); RT-04.V3 left open (the synchronized-writer-during-active-lease variant is not exercised).
- G-05.I2 warm benchmark (actual approved runtime): selected SmolLM2-135M Q4_K_M, started the managed CUDA runtime ("Started smollm2-135m-q4-k-m on port 8080", status LIVE, `GET /props` live), then the default warm benchmark through `/completion`: mean 999.00 tok/s (median 1001.80, range 991.32-1003.89) on the first candidate and mean 995.11 tok/s (median 996.42, range 990.93-997.99) on `f556fba6…`; sample mean equals the displayed mean in both runs. The identical-GPU mapping row stays untested by policy. "Accepted TLS/auth local profiles" was NOT exercised in this batch.
- G-05.I1 Job Object cleanup — DEFECT FOUND AND FIXED: with the pre-fix candidate `3fa11c20…`, `Stop-Process -Force` on the app left the managed `llama-server` (PID 1957196) alive and answering `/health` on 8080. Root cause: `process-wrap 10.0.0`'s std `JobObject` wrapper calls `make_job_object(handle, false)` (kill_on_drop = false), so its job never carries `JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE`; a second assignment into a shared kill-on-close job failed with `ACCESS is denied` on nested jobs. Fix (commit `e8a42ca`): `proc.rs` now owns containment itself — every contained child gets a fresh job with `JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE`, `TerminateJobObject` scopes kill/drop to that child's tree, and a child that cannot be contained is terminated rather than leaked. Regression: `contained_children_are_members_of_the_kill_on_close_job` (RED when the assignment is removed — recorded mutation run; GREEN with the fix) plus the retained descendant-termination tests (581 passed / 0 failed / 3 ignored). Packaged re-verification on `f556fba6…`: server live (PID 1691916, managed CUDA path) -> app killed -> `JOBOBJECT_KILLED_CHILD`, `/health` unreachable, no `llama-server` process remains.
- Remaining G-05 items (explicit, unresolved): NTFS rename/open-handle/hard-link cases; SQLite/catalog-DB locking and recovery; active cancellation (benchmark/health mid-run); high-contrast theme pass; Narrator/NVDA screen-reader pass (NOT RUN — no screen reader in this session); accepted TLS/auth local profile runs; DC-01.V3/DC-03.V3/DC-04.V2/DC-12.I4 packaged sequences. G-05 checkboxes stay unchecked until these close.
- Evidence paths: `.hermes-0.6/g05-install.log`, `.hermes-0.6/g05-managed-install2.log`, `.hermes-0.6/g05-health-baseline.log`, `.hermes-0.6/g05-launch-bench.log`, `.hermes-0.6/g05-jobobject-bench.log`, `.hermes-0.6/g05-tamper/` (marker, original bytes, absence of MARKER_RAN.txt), driver scripts `scripts/g05_state.mjs`, `scripts/g05_health.mjs`, `scripts/g05_managed_install.mjs`, `scripts/g05_launch_benchmark.mjs`.

#### G-05 packaged Windows wave — second batch (candidate `f556fba6…`, source `e8a42ca`)

- NTFS rename case (RT-04.V2 adjacency): with the legacy tree parked so only the managed copy existed, renaming the approved `llama-server.exe` away and running the seven-stage health produced FAILED with "Managed runtime cuda-13.3 is not installed or fails content verification" (`trust_failure`, later stages SKIPPED). Restoring the name returned `passed all seven managed-runtime health stages` immediately. Rename round-trip at the FS level also verified (`RENAME_ROUNDTRIP_OK`).
- DLL replacement case (RT-04.V2): replacing `llama-server-impl.dll` (8.9 MB) with the 4 096-byte marker exe failed health with the same content-verification refusal and `MARKER NOT EXECUTED`; restoring the DLL (sha256 `87c4e9d0…`) returned a full PASS. The content manifest therefore covers DLLs, not only the runtime binary.
- Legacy reuse verified end to end by observation: with BOTH copies present, CUDA health passed against the legacy copy (the app resolved the verified legacy install - RT-01 reuse); the CPU row's health also passed against the discovered legacy CPU install. One probe iteration misread this as a rename-test failure and is recorded rather than hidden.
- Active cancellation: the v2 benchmark run publishes a cancel handle - the `Cancel` control was enabled in flight and accepted a click at the first poll, returning the app to idle with no result recorded. Observed distinction (recorded): the legacy `Run benchmark` on the screen never enables `Cancel` during its run. The server-start cancel window was not caught (this model reaches live in under a second).
...
#### G-06 sandbox lifecycle re-bind at the current candidate (source `1a9ab98`, installers staged from the bundled 0.6.0 build)

Environment: Windows 11 (26100) host with Windows Sandbox (WDAGUtilityAccount clean account), Network enabled, candidate MSI/NSIS staged exactly as the release workflow stages them (`Localmotive_0.6.0_x64.msi` renamed from the `_en-US` build output, `Localmotive_0.6.0_x64-setup.exe`). Candidate digests: setup `a752a57b27fc8870660909e9be7db855395052baebd7241feb16e80b0acd063a`, msi `d298448c3274803202e46a4abc6687b9b1804be1921977764ddd26ac83e76e11`.

- Command (both baselines): `powershell -NoProfile -ExecutionPolicy Bypass -File scripts/sandbox/host-run-lifecycle.ps1 -Tag v0.6.0 -Version 0.6.0 -PreviousTag <v0.4.1|v0.5.0> -CandidateDir .hermes-0.6/g06-candidates -EvidenceName sandbox-preservation-<baseline>` with `GH_TOKEN` and `LOCALMOTIVE_SOURCE_REVISION=$(git rev-parse HEAD)` exported.
- Result: **PASS for both baselines** - `release-evidence/0.6.0/attestations/sandbox-preservation-v0.4.1.json` and `sandbox-preservation-v0.5.0.json`, each with `sourceRevision 1a9ab98c17352b77728b925b43228f2798e4c5ab`, `candidateDigests` as above, steps NSIS fresh install/launch/uninstall, MSI fresh install/launch/uninstall (with product-registration leftover check), NSIS update from the previous tag, and the new preservation step.
- Preservation (G-06.I2) now exercised per baseline: after the previous-tag NSIS install and a launch smoke, the harness plants a user-data canary file and a valid SQLite catalog mirror carrying a `canary_marker` table, upgrades to 0.6.0, launches, and requires both to survive; the mirror is collected to the evidence directory and the host verifier (`scripts/sandbox/verify_preservation.py`, stdlib sqlite3, read-only) must report `PRESERVE_PASS` for the marker and the user-data canary. Both baselines: host verification PASS, files retained (`sandbox-preservation-*.collected-mirror.sqlite`, `-collected-userdata.txt`, `-verify.log`).
- Installed-executable digests recorded per path: NSIS fresh = post-update = preservation-upgrade = `fa28dadd9629086a82dd624af3954ed37c49b74e7ce37991ef8c783dc5806eff`; MSI fresh = `70d2fee239d6170298d702f15aea82c2b0ede19199f2a460d73638170de67c21`; pre-update v0.4.1 `71e821fd…`, pre-update v0.5.0 `67f70fc3…` (both differ from the candidate digest, so the update provably replaced the executable). The harness now asserts NSIS-family digest identity and update replacement, and LOGS an explicit OBSERVED line when the MSI payload differs from the NSIS payload.
- **Open observation (not silently accepted):** the MSI-installed `Localmotive.exe` bytes differ from the NSIS-installed bytes for the same 0.6.0 build (deterministic across both runs; both report version 0.6.0). Cross-bundler byte identity is therefore not established for this candidate. Reproducer: install each candidate per account in a clean account and compare `Get-FileHash` of the installed `Localmotive.exe`. A host-side expected-digest model was tried first and invalidated - the Rust release build is not byte-reproducible between invocations (three different digests at identical size across the build and the two bundled payloads) - so the harness compares installed payloads to each other instead of to a host binary.
...
- **FE-05.V3 closed 2026-09-12:** the two-profile provenance walk with the saved-manifest route ran packaged (`scripts/g05_fe05v3.mjs`, 14/14 checks) - profile A (`-c 8192`) and profile B (`-c 4096`) measured with distinct identity keys and distinct persisted manifests, anchors added from the saved runs and the record list loaded through the compatibility key; see the re-bind section.
...
**6. Open-box reconciliation (18 open / 645 checked after this pass; MT-07.V2 and RT-06.V3 closed and G-05.I3 partially closed in the fifth-pass follow-up; the shipped-code delta is a `#[cfg(test)]`-only proc.rs change re-cut as freeze 7 `cb8ab64` and then freeze 8 `da091a4` (the first fixture diagnostic was disproved by CI and corrected; every process-affecting packaged check re-ran on the freeze-8 bytes; CI for `da091a4` green), with the canonical manifest regenerated at `release-evidence/0.6.0/qualification-manifest-0.6.0.json` bound to the freeze-8 source `da091a47ca65` and its three artifact digests; the freeze-7 set is labeled superseded, never rewritten). **Fresh-checkout validation (2026-09-12):** `scripts/verify_qualification_manifest.mjs` validates the manifest and all 18 referenced records (existence, JSON parse, status/pass-count equality, raw-sha or CRLF-normalized-sha equality, workflow-file digest, artifact bindings); a `git clone --depth 1` of the pushed revision runs it to `MANIFEST VERIFY PASS` (evidence `.hermes-0.6/fresh-clone-verify.log`), and the release gate suite pins the no-scratch-reference rule (mutation: a manifest referencing `.hermes-0.6` fails verification). The mt06 record was moved from gitignored scratch to the committed attestations copy, and the manifest now records `sha256_lf` per record plus the release-workflow revision separately from the artifact source (contract: artifact-producing source, workflow revision, and artifact digests stay distinct). A freeze-7 byte-doctored setup is refused at `resolve-installers` naming both digests (`88332e96...` vs `4ad8822e...`; evidence `sandbox-clean-account-lifecycle-negative-freeze7.json`).**
...
| RT-04.V2 | closed 2026-09-12 (fourth pass) | the delayed-download window now has its own harness: verifier-profile loopback seam on the health-model fetch plus `scripts/g05_rt04v2_delay.mjs` (controlled ≈4 MB/s fixture); packaged 17/17 — in-window swap denied (EBUSY), both-copies tamper refused with zero children, exact restore passes | none — evidence in `release-evidence/0.6.0/attestations/rt04v2-delayed-download-verification.log` |
| RT-06.V3 | closed 2026-09-12 (fifth pass) | maximum eligible backend set qualified on this host (4 of 7; the 3 vendor-mismatched backends are refused by the compatibility guard with named reasons); listing/selecting/launching measured with instrumentation counters; 12/12 checks | literal seven-backend install needs a three-vendor host - documented, not a code gap |
...
| MT-07.V2 | closed 2026-09-12 (fifth pass) | the five identity scenarios (CPU-only, changed CPU, fit-reduced context, draft settings, changed LoRA bytes) are covered as canonical-snapshot tests; mutation MUT-MT07 caught | none - residual live portability stays in the RT-06/S-25 hardware program |
...
| G-05.I3 | partially closed 2026-09-12 (fifth pass); residue environment-blocked | keyboard, reduced-motion, high-DPI/zoom and forced-colors legs PASS packaged (`verify_a11y.mjs` A11Y_PASS, evidence `g05-a11y-packaged-verification.log`); residue: a manual Narrator/NVDA listening pass and live-credential scenarios | owner session / authorized test account |
...
- Note: these defects were shipped in the working tree since the S-27.I2 adapter repoint and the MT-01/MT-07 work respectively; they were invisible to unit tests because the fixture contracts encoded the same wrong shapes. The packaged flow is the only level that caught all three, which is exactly what G-05 exists for. All three fixes change shipped behavior (`core.rs`, `measurement.rs`, `evidence-adapter.ts`), so the G-06 installer lifecycle and the earlier G-05 packaged cells bind to the PRE-FIX source `1a9ab98`: a final candidate must be re-cut from this fixed lineage and the lifecycle plus the critical packaged probes re-run before G-09/G-10; that re-bind is the next packaging milestone.
...
**Status:** Verified at the current candidate; installers clean-install/upgrade/uninstall and the persisted user-data/SQLite, canary-preservation scenarios PASS in Windows Sandbox for both (v0.4.1, v0.5.0) baselines; one open observation records that MSI-installed and NSIS-installed bytes differ for the same build · **Priority:** Release/verification gate derived from audit · **Owner:** Unassigned  
...
- Limits: V06-MT-04.V2 is satisfied at the decision seam: `combine_trial_outcome` takes the cleanup outcome as a parameter and its tests inject a failed cleanup (`a_measurement_is_withheld_when_its_server_could_not_be_stopped` - the measured result is withheld, never returned as success) and a failed measurement with successful cleanup (the original error survives). The packaged stop-latency, surviving-child, port-release and next-operation checks (V06-MT-04.V3) are V06-G-04/G-05 evidence. `core::benchmark_server_cancellable` returns on a 250 ms boundary after Stop.
...
- Residual limits: I3 completed with isolated persisted fixtures (v0.4.1 profile/settings and v0.5 SQLite migration scenarios); V2/V3 executed in the sandbox window - the 2026-09-12 preservation runs (v0.4.1 and v0.5.0 baselines) collected and host-verified the mirror/user-data canaries.
...
**G-03 addendum (2026-09-12, third pass): current-candidate outputs.** The closure above is the historical record at `065248a1`. For the current candidate the same gate family was re-run and recorded in `.hermes-0.6/g03-current/outputs.log` at freeze `7c32fa9`: `verify_versions.mjs 0.6.0` -> `"failures": []`; `npm run check` (tsc, Vitest, node suites, catalog validation, branding, research anchor, qualification, icons, production build) -> exit 0; `check_tracker.mjs` -> 72 findings (19/43/10), 111 packages, 663 checkboxes, 1051 audit links; `verify_workflow_gates.mjs` -> ok. Rust suite at the freeze: 600 passed / 0 failed / 7 ignored (`.hermes-0.6/freeze5-cargo-gates2.log`). Two latent defects were found and fixed while producing these outputs: the branding gate rejected `REPORT-0.6.md` and the two `g05_vitems_*` drivers that carried the retired product name contiguously (the drivers now assemble the legacy folder name from parts; the report no longer quotes it), and clippy rejected a dead field in the new slow-body fixture (fixed by asserting connection cleanup).
...
- Reduced motion: `prefers-reduced-motion` emulation applies and the stylesheet carries the rule (A11Y_PASS leg 2).
- High-DPI / zoom: deviceScaleFactor 2 with 150% page scale — no horizontal overflow (1280 <= 1280), 12 visible buttons, 0 unusable (A11Y_PASS leg 3, freeze-6 portable).
- Forced colors / high contrast: emulation applies; 28 controls with 0 wordless and every state tag carries words rather than colour alone (A11Y_PASS leg 4; navigate-to-Control added so the tag check is not vacuous). Evidence: `release-evidence/0.6.0/attestations/g05-a11y-packaged-verification.log`.
...
#### Candidate re-cut at f65dac1 (supersedes 065248a1/0322fbd2) — packaged probe batch

- Candidate: `npm run tauri build -- --no-bundle` -> `BUILD_EXIT 0`, portable exe 20 931 072 bytes, sha256 `3fa11c2078da7c4557cba83fa60bf5cf387f7763689865fef09a004b5151e68f`, source `f65dac1b54950a86d1a092e66a45ffe9b02604cf`, evidence `.hermes-0.6/final-build1.log`.
- Packaged probes on this candidate: `S27_RESULT PASS (7 checks)`, `S27TUNE_RESULT PASS (6 checks)`, CSP probe PASS (`remoteFetchBlocked` + `hostileTextInert` true), `RESPONSIVE_PASS` (incl. 200%/400% zoom, 0 px overflow), `A11Y_PASS`, `S23_RESULT PASS (7 checks)` (fixture `fixture-alpha` generated for the run), `S24_RESULT PASS (7 checks)`. Probe defect found and fixed honestly: `verify_s24_paths.mjs` hard-coded the earlier `s24-fixtures` folder name in its title assertion; the check now derives the expected basename from the passed fixture folder (behavioral mismatch, not a product defect).
- Binding note: the earlier G-05/G-06 lifecycle evidence (source `065248a1`, portable `0322fbd2`) predates the S-27/S-28/S-29 work; the candidate above is the current release candidate and G-05/G-06 must be re-run against it before G-09. S-25.I3 remains hardware-blocked and owned by G-08.

#### S-27 progress record — slices 1-3e: five Rust facades + About, Runtime and Tune screens (I1 complete; I2: remaining screens + panel acquisition remain)
...
- Slice 2 (Rust, I1): the managed-runtime setup/install/health command family (10 commands incl. `load_runtime_setup`, `fetch_runtime_catalog`, `install_managed_runtime`, `check_managed_runtime_health`, `repair_health_model`, `managed_runtime_root` and the three cancellation commands) moved from `lib.rs` to new `src-tauri/src/runtime_service.rs`; `ExclusiveOperation`, `RuntimeSetupResponse` and `AppState` became `pub(crate)` neighbors. Boundary regressions followed: three release-gates tests now read `runtime_service.rs`. Mutation QA1 (fetch-path exclusive lock dropped) and QA3 (install double-start refusal removed) initially PASSED — exposing two real gaps — so `runtime_service_source_tests` now pins the fetch lock before its fetch, the load-runtime-setup lock usage, and the install slot protocol (take → refuse second → run → clear); the exclusive-acquire behavior itself gained a direct unit test. QA1/QA3 are CAUGHT after the additions.
- Slice 3a (Rust, I1): the supervised server lifecycle family (`server_phase`, `start_server`, `start_server_worker`, `stop_server`, `server_status`, `read_server_log`) moved to new `src-tauri/src/server_service.rs`. The shared operation coordinator stays in `lib.rs` (`reserve_operation`, `OperationOwner`, `active_operation_owner`, `stop_owner_conflict`, `ManagedServer`, `status_from`, `StartingServer`, `ServerStatus`, `STARTUP_STOP_DEADLINE_SECS`, `spawn_server`, `wait_until_healthy_cancellable`, `update_launch_effective_context`, `bounded_log_tail` widened to `pub(crate)`) and the module consumes it rather than restating any ownership rule. Boundary regressions followed the code: the IPC-01 startup guard now reads `server_service.rs`, and the MT-05 "every launching command reserves its slot" guard reads the three service sources concatenated (server_service first, so its real functions precede test-text mentions — an ordering bug that failed the guard once and is recorded).
- Slice 3b (Rust, I1): the measurement/quality/calibration command family (764 lines: legacy and v2 benchmarks, cancel, manifest replay + worker, quality suite, ranking/join helpers, compatibility-key and calibration-model builders, `CalibrationRecords` + `calibration_storage_root`) moved to new `src-tauri/src/measurement_service.rs`. A single `crate::ALL_SOURCES` test constant now concatenates every source file (service modules first, so real function bodies precede test-text mentions) and ALL TEN source guards read it — the earlier per-file reads broke five guards as code moved, which is exactly the drift this constant prevents; two ordering mistakes in the concatenation (real-vs-test-text precedence) were caught by the failing guards and fixed.
- Slice 3c (Rust, I1): the AI tuning command family (`tune_disclosure_list`, `start_tuning`, `clear_tuning`, `cancel_tuning`, the `LiveBench` bench adapter, `TuningRequest`/`TuningProgress` and `system_ram_bytes`) moved to new `src-tauri/src/tune_service.rs` — the last supervised job-lifecycle owner. The tuning engine (`tune.rs`), cloud advisor and shared operation coordinator remain authoritative.
- Slice 3c V1: mutation QC1 (start_tuning reserving the wrong owner) CAUGHT by the MT-05 guard. Mutation QC2 (cancel storing `false` instead of `true`) initially MISSED every test — a real coverage gap; fixed with `cancel_tuning_with` + `cancel_tuning_sets_the_flag_the_session_polls` (RED-before-GREEN), after which QC2 is CAUGHT. Clippy findings (mid-file test module, literal-bool asserts) fixed. Gates: fmt PASS, clippy 0, `cargo test` 580/0/3.
- REMNANTS in I1 (explicit): none of the supervised job-lifecycle owners remain in `lib.rs`; the remaining command bodies are the cloud/credential family, which is a separate surface and stays put unless the audit requires otherwise. I2 (further screen/hook extraction) remains open.
...
- Slice 3d (FE, I2): the Runtime manager screen is extracted to `src/screens/RuntimeScreen.tsx` (402 lines) as a presentational component with an explicit typed props interface (36 props; acquisition, persistence and decision logic stay in `App.tsx`). `src/screens/RuntimeScreen.test.tsx` pins the visible contract on public React/DOM interfaces: managed install identity for an installed runtime (RT-06), the empty-catalog message without a loading indicator, and the health-cancel pending state. Mutation "drop the installKey render" CAUGHT by the contract test. Because 30 release-gates source guards read `src/App.tsx`, they now read a shared `frontendSources()` concatenation (screen modules first, then App.tsx) — the same drift-prevention pattern as the Rust `ALL_SOURCES`; two guards that matched moved JSX were repointed to accept the prop-indirected forms rather than weakened.
- Slice 3d packaged evidence: portable exe rebuilt (`BUILD_EXIT 0`, 20 931 072 bytes, sha256 `04760905397debe6f1483a5026cb3b2aec1a2b3fa2db2d75e513ba841110fb6b`), `scripts/verify_s27_runtime.mjs` → `S27_RESULT PASS (7 checks)` against the real WebView2 window (nav, heading, subtitle, refresh action, managed-runtime panel, path actions, zero uncaught exceptions). One probe iteration: `innerText` returns CSS-uppercased headings, so the heading checks compare case-insensitively (recorded, not silently adjusted around). Runtime screen extraction carries no packaged mutation run (a pure move; the mutation coverage is the unit-level installKey render check plus the guard suites). Gates: TSC PASS, Vitest 119, node 145/145, `npm run build` OK (351.04 kB), `cargo test` 580/0/3.
- Slice 3e (FE, I2): the Tune screen is extracted to `src/screens/TuneScreen.tsx` (284 lines) as a presentational component with an explicit typed props interface (49 props, `TuneRunIdentity` for run/report identity). The body keeps the original JSX byte-for-byte and destructures the props at the top, so TS narrowing behaves exactly as before per item. `src/screens/TuneScreen.test.tsx` pins the visible contract: the persisted disclosure radio pair (S-20), the readiness gate on Auto-tune, and the blocker line. Mutations QT1 (disclosure checked-state ignored) and QT2 (readiness gate removed) are both CAUGHT. `TuneScreen.tsx` joined `frontendSources()`.
- Slice 3e packaged evidence: portable exe rebuilt (`BUILD_EXIT 0`, 20 931 072 bytes, sha256 `bc6ee640a9a8be951ff278369f0c0d495e9e1433898e676b23f1c4808cdca734`); `scripts/verify_s27_tune.mjs` → `S27TUNE_RESULT PASS (6 checks)` (nav, heading, provider tablist + key input, disclosure radios, Auto-tune action, zero exceptions) and the slice-3d `scripts/verify_s27_runtime.mjs` re-PASSED (`S27_RESULT PASS (7 checks)`) against the same binary. Two probe iterations recorded: the nav label is "AI Tune" (not "Auto-tune"), and the `aria-label` "Tuning readiness" is not text content — the heading check uses visible copy; `document.body.textContent` is polled because the nav click returns before the frame settles.
- Slice 3f (FE, I2): the Dashboard, Benchmark and Inventory screens are extracted to `src/screens/DashboardScreen.tsx`, `BenchmarkScreen.tsx` and `InventoryScreen.tsx` as presentational components with explicit typed props (an `EvidenceRun` shared type and a `PathText` module extraction make the boundaries importable without cycles; `PathText` moved out of `App.tsx` byte-for-byte). The FE-05 guard was repointed to assert the always-mounted section renders `BenchmarkScreen` and that the panel's `onRunStateChange` arrives as a prop — the mount semantics the guard exists to protect are unchanged. `frontendSources()` grew the new screen modules. Honest incident: the first Benchmark splice used a fuzzy `String.indexOf` anchor and swallowed part of the Inventory block; the worktree was restored from `HEAD` (`git checkout HEAD -- src/App.tsx`), the splice was redone with exact line arithmetic, and the boundary was verified (`Logical inventory` markers present, then `tsc`/`npm test` green). Gates after slice 3f: TSC PASS, Vitest 122, node 145/145.
- Slice 3g (FE, I2): the Catalog and Profile screens are extracted (`src/screens/CatalogScreen.tsx` 293 lines / 49 props; `ProfileEmptyScreen.tsx` + `ProfileScreen.tsx`). Every view in `App.tsx` is now a presentational component; `App.tsx` holds state, handlers and view switching by design (1729 lines, down from 2792 at the start of slice 3d). The evidence panel's acquisition is separated behind a typed `EvidenceAdapter` interface (`src/evidence-adapter.ts`, 20 commands): all twenty `invoke<...>` call sites in `V03EvidencePanel.tsx` became `adapter.<method>(...)` calls; the panel takes an optional `adapter` prop (default `tauriEvidenceAdapter`), `BenchmarkScreen` forwards it, and `App.tsx` injects `tauriEvidenceAdapter` explicitly. The FE-07 guard now pins panel→adapter and adapter→`cancel_benchmark`, and the existing calibration/preflight suites exercise the whole flow through the default adapter against the mocked IPC unchanged. All `frontendSources()` screen lists updated (guards for cooldown/rich-filters/FE-16/FE-05 repointed where the code moved). Gates: TSC PASS, Vitest 122, node 145/145, `npm run build` OK (357.13 kB).
- I2 closure: the audit's separation goal is met — independent screens, typed operation state (`EvidenceRun`), and presentational components separated from acquisition and persistence (the adapter seam).
- Candidate-binding note: the G-03 candidate `0322fbd2…`/G-05/G-06 evidence bind to source `065248a1…`; HEAD has moved through S-27/S-28/S-29, so the release candidate must be re-cut and those records re-bound before G-09. The current packaged binary for S-27 probes is `bc6ee640…` (pre-3g); slice 3g awaits its own packaged verification on the next candidate build.
