...
- REMNANTS in I1 (explicit): the supervised job-lifecycle facade for the server/benchmark/tuning coordinator is the remaining extraction; next slice = move the `OperationCoordinator`-backed start/stop/benchmark command bodies behind a service module.
