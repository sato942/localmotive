...
**Current checkpoint (2026-09-12, third pass; candidate freeze `3a2b06e`; docs/evidence tip follows on `main`):** the third pass closed DC-04.V2 through the supported command path (13/13 against controlled loopback bytes, no override UI required), corrected two release-path defects (the hardcoded `v0.5.0` publish guard that would have skipped the v0.6.0 publication, and publication ordering that ignored the lifecycle verdict), closed G-10.I2/I3/V1 with a traceability validator (`node scripts/verify_tracker_links.mjs`: 1397 links, 639 checked / 24 open, register ids reconciled), and recorded the full-SHA candidate lineage `a0ed247` -> `6384df0` -> `57bde64` -> `3a2b06e` (section "Third pass..." below). Gates on the freeze: `npx tsc --noEmit` 0; Vitest 141; node gates 157; `cargo fmt --check` 0; clippy `-D warnings` 0; `cargo test` 595 passed / 0 failed / 7 ignored. The candidate is re-cut at `3a2b06e` (portable `fbbd2a1a…`, msi `1d217350…`, setup `b83afa2c…`) and the third-pass packaged set, witnesses, DC-04 run, lifecycle chain and isolated verify_041 are recorded below. Open boxes are exactly the owner-gated GH-01/GH-02/GH-03 rows, GH-06.V3, G-09 rows and G-10.I1, plus the environment-blocked rows (RT-04.V2, RT-06.V3, DC-12.V3, MT-07.V2, S-25.I3, G-05.I3) - each individually categorised in the third-pass reconciliation, with FE-05 held open for the release decision on its stated criteria.
