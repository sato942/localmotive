...
| R11 | Packaged cancellation driver measures process names rather than owned PIDs, treats enumeration errors as zero, omits the run record/terminal outcome and restart-overlap scenario, and its log is not in the committed evidence | V06-MT-06.V3/G-05; audit MT-06/G-05 | **Open — in progress** | Driver rewrite lands with the campaign; result file committed under `release-evidence/0.6.0/`. |
