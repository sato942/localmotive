...
- [ ] Run allowlist dry-run against HF.
- [ ] Record repo count, file count, artifact bytes.
- [ ] Record decision on size limit and format in this file.
- [ ] Decide gzip JSON versus signed SQLite network drop.
- [ ] Keep current 4 MiB body and 5 MiB cache limits until the decision lands.
...
- Allowlist config: `catalog/providers.json` (planned, does not exist yet).
...
- Ambiguous names need resolution in PR: `IFM` and any missing HF org. Do not invent.
...
- Size limits now: body 4 MiB, cache 5 MiB. Size decision from dry-run: PENDING.
- Format decision: PENDING dry-run. Use gzip JSON if it fits. Else use signed SQLite artifact as network drop. Keep no server.
