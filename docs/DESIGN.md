...
- **Numeric profile, Tune and benchmark fields:** text inputs with a numeric keyboard (`inputMode="numeric"`), parsed by `parseNumericText` through `useNumericText`; Rust-compatible bounds enforced by pre-action checks (`profileNumberError`, `tuningWorkloadError`, `validateWorkload`), never by the widget alone. Keep incomplete input visible. Show input problems as text. Keep an explicit zero distinct from a blank field.
