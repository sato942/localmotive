...
| V06-RT-04.V2 | checked | Fourth pass (2026-09-12): packaged 17/17 with a controlled ≈4 MB/s loopback fixture streaming the pinned model. In-window swap attempt denied (EBUSY via the read-shared execution lease); tampering both verified runtime copies outside the window refused with `trust_failure` and zero children; exact restore (`87c4e9d0…`) returns a full seven-stage PASS. Evidence: `release-evidence/0.6.0/attestations/rt04v2-delayed-download-verification.log`; seam `download::rebase_download_url` (loopback-only plain HTTP under the verifier profile). |
| V06-RT-06.V3 | closed-deferred-far-future 2026-09-21 (owner scope; not executed, not PASS) | Owner: agent. Reason: measurement requires all seven backends installed on a representative host; this host has one verified CUDA backend. 0.6 will not obtain the required hardware; out of scope, not an open gate. Residual risk: low (listing/launch measured qualitatively through G-05; `installKey` and bounded reads ship with tests). Workaround: none. Follow-up: only if the owner later provides the environment and asks. Evidence gap: bytes-hashed/job-count/elapsed numbers across seven backends. |
...
| V06-MT-06.V3 | checked | G-05 batch 4: accepted TLS/key profile against the packaged target runtime - `https://127.0.0.1:8080` listener, `/props` 401 without key / 200 with key, app live, app benchmark over TLS mean 1004.76 tok/s (median 1006.10). Fourth pass extension (2026-09-12): three unit resource tests (`a_cancelled_body_read_resolves_worker_ownership_without_duplicate_requests`, `a_cancelled_call_lets_a_short_body_finish_once_on_the_owned_connection`, `repeated_cancel_restart_cycles_keep_workers_bounded_and_requests_exact`; slow-body fixture, worker-liveness counter; mutations MUT-W1/W2 caught) and a packaged six-cycle resource driver (`scripts/g05_mt06_cycles.mjs`, 37/37 checks PASS: in-flight cancel per cycle, `llamacpp:requests_processing` drains to 0, one owned child during and zero after stop, listener released). |
...
| V06-MT-07.V2 | closed-deferred-far-future 2026-09-21 (owner scope; not executed, not PASS) | Owner: agent. Reason: CPU-only and changed-CPU identities are distinguished at the unit level, but 0.6 will not obtain a CPU-only machine to prove calibration identity on that class. Out of scope, not an open gate; no claim that calibration works on CPU-only hosts. Residual risk: low-medium (snapshot schema is field-driven). Follow-up: only if the owner provides the machine and asks. Evidence gap: real-machine identity separation. |
...
| V06-DC-12.V3 | closed-deferred-far-future 2026-09-21 (owner scope; not executed, not PASS) | Owner: agent. Reason: OS crash/power-loss testing cannot be run safely on the live verification host. 0.6 will not obtain a storage-class rig or power-loss facility; out of scope, not an open gate. Residual risk: low (`.part.json` resume state and journaled publication ship with tests). Follow-up: only if the owner later provides the environment and asks. Evidence gap: power-loss/OS-crash results. |
...
| Item | Owner | Reason | Residual risk | Workaround | Follow-up | Evidence gap |
|---|---|---|---|---|---|---|
| V06-GH-01.I3 | owner (GitHub configuration) | Superseded 2026-09-20: both rulesets are active as of the live API - `main-pr-check` 23218749 (refs/heads/main, required context `pr-check` with strict policy, owner-only bypass) and `immutable-release-tags` 23218751 (refs/tags/v*, update plus deletion). `branches/main.protected` is false (expected; rulesets are the gate). | Low: `pr-check` is a merge gate; owner holds the sole documented bypass | Manual review before merge | Owner-bypass edit applied 2026-09-20 | Effective settings readback |
...
| V06-GH-01.V2 | owner | Superseded 2026-09-20 by owner disposition: a dedicated failing-check PR is not a release blocker for a solo repo. Optional owner work only; do not merge a failing revision if executed. | Low | None | Owner authorizes a failing-check PR only if a real regression needs it | This disposition |
| V06-GH-01.V3 | owner | Superseded 2026-09-20 by owner disposition: waived as an access prerequisite. No Write collaborator exists. Effective readback recorded (main-pr-check 23218749, owner-only bypass sato942 id 2147851). V3 reopens when the first Write collaborator is invited. | Low | None | Direct-push refusal with that identity before the grant takes effect | Ruleset readback |
| V06-GH-02.I3 | owner (GitHub configuration) | Superseded 2026-09-20: the tag ruleset is active as of the live API - `immutable-release-tags` 23218751 (refs/tags/v*, update plus deletion, no bypass). Never retarget a used tag (already policy). | Low | Never retarget a used tag (already policy) | Readback recorded 2026-09-20 | Ruleset readback |
| V06-GH-02.V3 | owner | Split 2026-09-20: tag-protection readback done (same live JSON as I3 — `immutable-release-tags` 23218751, active, no bypass). Complete candidate flow vs checkout / inventory / packaged evidence / publication metadata stays open, blocked on U06-06 then U06-08; no fixture stands in as publication proof. | Low | Local harness + fixtures cover the semantics | Executes at G-09 | Release-run comparison |
...
| V06-RT-06.V3 | release manager | Superseded 2026-09-21: CLOSED-DEFERRED-FAR-FUTURE, out of 0.6 scope. Seven backends are not installed; single-backend behavior is recorded | Low | Install backends on demand | Multi-backend host session | Seven-backend metrics |
...
| V06-DC-12.V3 | release manager | Superseded 2026-09-21: CLOSED-DEFERRED-FAR-FUTURE, out of 0.6 scope. Write-path throughput is measured (DC-12.I4) | Low (resume semantics tested at process level) | Journaling/`.part.json` design | Fault-injection lab | Crash/scaling matrix |
...
| V06-MT-07.V2 | release manager | Superseded 2026-09-21: CLOSED-DEFERRED-FAR-FUTURE, out of 0.6 scope. The snapshot tests cover the classes at unit level | Low | Unit-provided coverage | Future hardware session | Machine-class matrix |
...
| V06-G-05.I3 | owner/release manager | Superseded 2026-09-21: leftovers CLOSED-DEFERRED-USER-REPORTS. Keyboard, engine-level high contrast, zoom/DPI and reduced motion are done and stand; Narrator/NVDA and live cloud/HF credential scenarios are out of the 0.6 campaign — further defects arrive via real-user issues | Low-Medium | Automated a11y PASS stands as-is | User-reported issues | Screen-reader + live-provider evidence |
...
Owner configuration package (prepared, not applied - repository settings are owner actions): create a main-branch ruleset requiring the `pr-check` check with no bypass actors for ordinary contributions, forbidding force pushes and deletions; create a tag ruleset for `v*` blocking updates and deletions. Exact `gh api` payloads are kept with the G-09 owner package. Update 2026-09-20: both rulesets are now applied and active as of the live API (`main-pr-check` 23218749 with required `pr-check` and owner-only bypass sato942; `immutable-release-tags` 23218751 with no bypass). The no-bypass-for-ordinary-contributors intent stands; the owner-only bypass is the documented solo/emergency path.
...
# historical binary producer e9a36b3aa075d72713d88af4cc74d64d561f12f8
# (portable d6a6ef663c925f9605f06ea2957ef95409dace00d798ebc28e5666249fd59427,
# setup b935033d2ca2dbaf595c0a20e629053f0b4097ebe4b1059c351d140530ab8652,
# msi bc5c6aae332487430c1d7a238b668e3919f1514f713dd0c9d1ab80e248313b9b);
# those digests describe the artifacts that run produced, not a promise about
# a future build. The repair changes harness/verifier/workflow behavior with
# no product-source changes; the release verification workflow builds at its
# resolved tag revision and qualifies that run's actual outputs, and promotion
# consumes the selected successful verification run's exact qualified bytes.
# A future run's outputs must not automatically inherit the historical
# digests above. Workflow/harness 362843cb7c1bb36b680729deeba4714e19103f6a
...
Expected RELEASE ASSETS (the six files the release carries, pinned by the read-back): the NSIS setup, the portable exe, the MSI, `SHA256SUMS-<version>.txt`, `packaged-verification-<version>.json`, and `candidate-inventory-<version>.json`. The catalog matrix, the lifecycle attestations, the promotion decision record and the read-back itself are WORKFLOW ARTIFACTS retained for 90 days under the run, not release assets. **Current candidate (freeze 9, 2026-09-13):** source `eb01bc9f97417e74ce7de7da85c86582f42ceb67`; portable `97e1a4fb2c7622c211123c8e99abedc7e08f648ef3b6a0b8aa543121e6cad8c5`, setup `8398cf8a25949049b939e33aa2613330e41e7882aa951b868d064bc4fbc73bef`, msi `2c00e0d3f9e2dc050fd3ec5fd02bc9f6b0923605b6d715b20a63f8292e30d11b`; staged inventory `candidate-inventory-0.6.0.json` (PASS, 3 artifacts). Freeze-9 supersedes freeze 8 (`da091a4`) because the R16 remediation changed product code, the release path and the qualification tooling; the freeze-8 manifest, inventory and records are preserved unlabelled under `release-evidence/0.6.0/history/freeze8-da091a4/`, and every freeze-9 record binds to `eb01bc9f9741` through the inventory. Lifecycle: the v0.5.0 upgrade leg and the v0.4.1 preservation leg re-ran natively on the freeze-9 bytes in Windows Sandbox (both PASS, including the v0.5.0 user-data and catalog-mirror preservation and the v0.4.1 cache preservation, with the installed version and digest checks); the v0.4.0 upgrade and the v0.5.0 preservation legs are carried forward from freeze 8 through the verifier-enforced carry-forward ledger, which re-checks their digests against the committed freeze-8 producer inventory and records the source-delta reason. The two remaining sandbox re-runs stay an open environment prerequisite because Windows Sandbox boots only intermittently on this host. The installed NSIS payload digest is `7ec790cf67cc70e3051621b6ef36dc494a16f39d8cd35db495dcdb19a4b87e29` and the installed MSI payload digest is `3a736bb9ee2928f02d73e718d51092cdc126e7920c22392eff33f909b9aa4f08` (byte-identical to the portable apart from the 3-byte bundle-type marker each bundler patches); both payloads were launched from their extracted bytes and evaluated over CDP (8 navigation buttons, managed controls rendered). Freeze-10 supersedes the freeze-9 candidate above: producer source `e9a36b3aa075d72713d88af4cc74d64d561f12f8` (portable `d6a6ef663c925f9605f06ea2957ef95409dace00d798ebc28e5666249fd59427`, setup `b935033d2ca2dbaf595c0a20e629053f0b4097ebe4b1059c351d140530ab8652`, msi `bc5c6aae332487430c1d7a238b668e3919f1514f713dd0c9d1ab80e248313b9b`); the F9 repair pass (`362843c`) changes harness/verifier/workflow only, with no product-source changes; the historical freeze-10 bytes above are retained as evidence of that run, and the workflow/harness revisions and manifest rebind are recorded at `9e8582f` with the tag target in section 2. A future verification run qualifies its own outputs; those outputs must not automatically inherit the historical digests.
...
The remaining High-finding verification items were executed packaged and every one is now closed (2026-09-12 re-bind): FE-01.V3, FE-02.V2, FE-03.V1/V3, FE-04.V2, FE-05.V1/V2/V3, FE-07.V2, FE-16.V1/V3, IPC-01.V2, MT-04.V2, MT-05.V3, QD-02.I4, QD-03.V3, RT-07.V2, GH-06.V2 (see the tracker's re-bind campaign record). Still open with explicit dispositions: the GH-01/GH-02/GH-03 owner-gated items (rulesets, authorized PR runs, release-run observation; GH-02.I3 closed 2026-09-20 on live readback, V06-GH-02.V3 readback done with the flow compare still open), GH-06.V3 and the G-09 rows plus G-10.I1 (owner authorization), the far-future-closed RT-06.V3/DC-12.V3 (owner scope 2026-09-21; not executed, not PASS), the far-future-closed MT-07.V2 (owner scope 2026-09-21; not executed, not PASS), S-25.I3 and G-05.I3 rows, and the final G-08 reconciliation held open until the unresolved High findings close. RT-06 freeze-9 update: the installed-backend scope is now recorded per backend (installed/selected/launched with the verification root) and the seven-installed-backend criterion remains explicitly not satisfied on this single-vendor host. DC-04.V2 was executed and closed in the 2026-09-12 third pass; G-10.I2/I3/V1 were closed the same day once their requirements were met. No High finding has an unresolved item without a visible disposition, and FE-05 stays finding-level open for the release decision on its stated completion criteria.

## Freeze-10 repair pass - current disposition (F9-01..F9-06)

The freeze-9 review findings were reconciled against candidate `e9a36b3aa075d72713d88af4cc74d64d561f12f8` (freeze 10). The rows below are the CURRENT labels; the earlier freeze-8/freeze-9 status text elsewhere in this document is historical and is not rewritten.

| F9 | Trace | Current disposition | Evidence |
| --- | --- | --- | --- |
| F9-01 | MT-05/MT-06; R11 | fixed and verified: `scripts/lib/mt06_verdicts.mjs` rules plus a driver that attempts the replacement on one surface per scenario while the previous request is observed active (`requests_processing=1` immediately before the invocation), feeds the immediate after-sample (`processingAfterInvocation`) into the same strictly bracketed coverage classification (after `1` stays active; a valid count above `1` is `overlap` and fails; after `0` is a drained interval and null/missing/error is an unavailable observation, both not-exercised with the actual reason preserved in the verdict), opens the drain overlap window with that sample without letting an incomplete sample override genuine drain-loop overlap, preserves the original run's identity and outcome, and never credits missing evidence with active coverage | `scripts/tests/mt06_verdicts.test.mjs` 9/9 incl. the after-sample replay (`2`/`3` fail both scenarios; `0`/null/missing/error are NOT-EXERCISED for both; recorded `1` stays PASS) with RED proof against the prior classification; packaged `mt06-cycles-result.json` 75/75, 0 not exercised, replayed through the corrected wiring with identical verdicts (replay, not a new packaged run) |
| F9-02 | MT-05/MT-06; R04 | fixed and verified: the fallible client is constructed before the active slot is published (`publish_benchmark_slot`), with a behavioural regression for construction failure and a command-level source gate | `measurement_service.rs` tests `f9_02_*` (mutation-proven); packaged ownership paths re-ran in MT-06 |
| F9-03 | GH-02; R08/R12 | fixed and verified: the producer schema is explicit and required; source revision, artifact name/size/digest and per-check status are compared with the inventory and the aggregate | `scripts/tests/qualification_manifest.test.mjs` 28/28 incl. the genuine-record controls |
| F9-04 | GH-02/GH-03/GH-06; R13/R14 | fixed and verified: producer filename matches the promotion contract, qualification consumes this run's lifecycle records, package failures retain diagnostics and report missing outputs, and the qualification step stages the freshly produced `artifacts/packaged-verification-<version>.json` over the attestation input (`--stage-packaged-verification`; a missing stage file fails instead of binding a stale copy) | `scripts/tests/workflow_integration.test.mjs` 10/10 incl. the staged-record transfer (fresh timestamps pass with no prepopulated copies; missing stage file fails) and the workflow-text pin; manifest + promotion rehearsal green on the freeze-10 layout |
| F9-05 | GH-04/G-06; R07 | independent work complete, named external verification still blocked: fixtures derive from the released v0.4.1 keys/shapes, the sandbox seeds them through the baseline build and reads them back, the cache verifier requires `schemaVersion` 1 and rejects a `models` field that is not an array (the released v0.4.1 `Catalog` carries `models: Vec<CatalogModel>`) | `scripts/tests/preservation_verifier.test.mjs` 8/8 incl. the `models:string` rejection; `src/App.v041Recovery.test.tsx` 3/3; native sandbox run still blocked (Windows Sandbox cannot boot on this host) |
| F9-06 | RT-06/G-08 | reconciliation complete: the four-backend scope (22/22) is current; the seven-installed-backend acceptance portion is CLOSED-DEFERRED-FAR-FUTURE 2026-09-21 (owner scope; not executed, not PASS) in the tracker, this review and the report | `docs/history/TODO-0.6.md` V06-RT-06.V3 rows; `REPORT-0.6.md` (this pass's paragraph); `rt06-all-backends.json` 22/22 on freeze-10 |

Owner-gated and environment items are unchanged: tag creation/push, publication/promotion, rulesets, the hosted PR campaign, the MT-07 physical-hardware program and the two native sandbox reruns. The three-vendor RT-06 host and the DC-12 storage/OS-crash rig are CLOSED-DEFERRED-FAR-FUTURE 2026-09-21 (owner scope), not unchanged-open items. Code signing is not pursued: releases ship unsigned as standing policy.
