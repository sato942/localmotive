...
| F9-06 | RT-06/G-08 | reconciliation complete: the four-backend scope (22/22) is current; the seven-installed-backend acceptance portion is left open and environment-blocked in the tracker, this review and the report | `docs/history/TODO-0.6.md` V06-RT-06.V3 rows; `REPORT-0.6.md` (this pass's paragraph); `rt06-all-backends.json` 22/22 on freeze-10 |

Owner-gated and environment items are unchanged: tag creation/push, publication/promotion, rulesets, the hosted PR campaign, the three-vendor RT-06 host, the MT-07 physical-hardware program and the two native sandbox reruns. Code signing is not pursued: releases ship unsigned as standing policy.
