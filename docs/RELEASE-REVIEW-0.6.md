...
| --- | --- | --- | --- |
| F9-01 | MT-05/MT-06; R11 | fixed and verified: `scripts/lib/mt06_verdicts.mjs` rules plus a driver that attempts the replacement on one surface per scenario while the previous request is observed active (`requests_processing=1` at the invocation), preserves the original run's identity and outcome, and reports an inactive attempt as not exercised | `scripts/tests/mt06_verdicts.test.mjs` 6/6; packaged `mt06-cycles-result.json` 75/75, 0 not exercised |
...
| F9-04 | GH-02/GH-03/GH-06; R13/R14 | fixed and verified: producer filename matches the promotion contract, qualification consumes this run's lifecycle records, package failures retain diagnostics and report missing outputs | `scripts/tests/workflow_integration.test.mjs` 8/8; manifest + promotion rehearsal green on the freeze-10 layout |
| F9-05 | GH-04/G-06; R07 | independent work complete, named external verification still blocked: fixtures derive from the released v0.4.1 keys/shapes, the sandbox seeds them through the baseline build and reads them back, the cache verifier requires `schemaVersion` 1 | `scripts/tests/preservation_verifier.test.mjs` 7/7; `src/App.v041Recovery.test.tsx` 3/3; native sandbox run still blocked (Windows Sandbox cannot boot on this host) |
