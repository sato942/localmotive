...
# producer (binaries) source e9a36b3aa075d72713d88af4cc74d64d561f12f8
...
# reused: this pass changes no build input, so a rebuild from the tag commit
...
| --- | --- | --- | --- |
| F9-01 | MT-05/MT-06; R11 | fixed and verified: `scripts/lib/mt06_verdicts.mjs` rules plus a driver that attempts the replacement on one surface per scenario while the previous request is observed active (`requests_processing=1` immediately before the invocation), feeds the immediate after-sample (`processingAfterInvocation`) into the same coverage classification with an `overlap` outcome that fails the scenario, opens the drain overlap window with that sample, preserves the original run's identity and outcome, and reports an inactive attempt as not exercised | `scripts/tests/mt06_verdicts.test.mjs` 7/7 incl. the after-sample replay (`processingAfterInvocation=2` fails both scenarios through the classify-then-evaluate wiring; recorded `1` stays PASS); packaged `mt06-cycles-result.json` 75/75, 0 not exercised, replayed through the new wiring with identical verdicts |
