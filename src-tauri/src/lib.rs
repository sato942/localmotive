...
fn wait_until_healthy_inner(
    ...
    // Transport errors are expected while a server boots; an ownership
    // mismatch is anomalous. The timeout prefers the anomaly, then the
    // freshest transport error.
    ...
    loop {
        ...
        }
        {
            ...
            let probe = client.get_bytes("/health", probe_budget);
            if let Err(error) = &probe {
                // A certificate verification failure cannot become healthy by
                // waiting; surface it immediately with the recovery step, and
                // otherwise remember the last transport error so a timeout
                // names why the wait never succeeded.
                if let Some(guidance) = crate::local_client::certificate_guidance(error) {
                    return Err(launch_failure(
                        "health_tls",
                ...
                last_transport_error = Some(error.chars().take(240).collect::<String>());
            }
        ...
        if Instant::now() >= deadline {
            let detail = last_health_error
                .as_deref()
                .or(last_transport_error.as_deref())
...
mod release_security_tests {
    ...
    #[test]
    fn health_timeout_names_the_last_transport_error() {
        // Regression: the startup wait discarded client transport errors, so a
        // timeout said only "did not become healthy" and the actual reason was
        // invisible. A silent server (accepts, never answers) is a plain
        // example: the wait must keep retrying, then report the last error.
        let (port, _seen) = crate::local_client::tests::serve_plain(Vec::new(), false);
        ...
        let error = wait_until_healthy(&mut child, &client, "", Duration::from_millis(2500))
        ...
        assert!(
            evidence.message.contains("last validation error"),
            "the timeout must name why it waited: {}",
            evidence.message
        );
    }
    ...
    fn health_wait_fails_fast_on_a_ca_certificate_with_actionable_guidance() {
        // Regression: a certificate pair generated by `openssl req -x509`
        // defaults to basicConstraints CA:TRUE. webpki refuses such a pair as
        // the server certificate ("CaUsedAsEndEntity"). Before this test the
        // startup wait retried until its ten-minute timeout and the failure
        // message did not name the certificate problem at all; users saw a
        // launch that neither succeeded nor explained itself.
        let dir = std::env::temp_dir().join(format!(
            "localmotive-ca-true-fixture-{}",
            std::process::id()
        ));
        std::fs::create_dir_all(&dir).unwrap();
        let cert_path = dir.join("ca-true-cert.pem");
        let key_path = dir.join("ca-true-key.pem");
        std::fs::write(&cert_path, crate::local_client::tests::CA_TRUE_CERT).unwrap();
        std::fs::write(&key_path, crate::local_client::tests::CA_TRUE_KEY).unwrap();

        let port = crate::local_client::tests::serve_tls(
            crate::local_client::tests::CA_TRUE_CERT,
            crate::local_client::tests::CA_TRUE_KEY,
            b"HTTP/1.1 200 OK\r\nContent-Length: 2\r\n\r\nok".to_vec(),
        );
        let profile = crate::core::LaunchProfile {
            host: "127.0.0.1".into(),
            port,
            ssl_cert_file: cert_path.to_string_lossy().to_string(),
            ..Default::default()
        };
        let client = crate::local_client::LocalHttpClient::from_profile(&profile).unwrap();
        ...
        let error = wait_until_healthy(&mut child, &client, "", Duration::from_secs(30))
            .expect_err("a CA-marked server certificate must be refused");
        ...
        let evidence: LaunchFailureEvidence =
            serde_json::from_str(&error).expect("structured launch evidence");
        assert_eq!(
            evidence.phase, "health_tls",
            "expected an immediate TLS verdict, got: {}",
            evidence.message
        );
        assert!(
            evidence.message.contains("CA"),
            "the message must name the CA problem: {}",
            evidence.message
        );
        assert!(
            evidence.message.contains("CA:FALSE"),
            "the message must carry the recovery instruction: {}",
            evidence.message
        );
        assert!(
            elapsed < Duration::from_secs(10),
            "the TLS verdict must not wait for the timeout: {elapsed:?}"
        );
    }
