...
**0.6.1 campaign IN FLIGHT, BLOCKED on host network (2026-09-22).** Bump commit `4b31431d28d1503efc7b80a46c77ad2c3d54f082` is on origin (`release: version 0.6.1`, six version fields green). Local qualify partial on that SHA: build + inventory, installer payload, mt06 75/75, rt06 22/22 + log, a11y PASS, witnesses ALL PASS, dc04 13/13, rt04 17/17. Packaged matrix failed 3x and the 4 lifecycle legs are pending: the host IPv6 route is broken (IPv4 reaches GitHub in 80 ms; IPv6 stalls 15 s per connection then fails; the app's catalog/install timeouts blow). No tag created. Nothing published. Resume with the matrix + legs + manifest + gates + tag when the route recovers.
