...
- `L-29 | 2026-09-26 | 447d304 (merge of c4ffa81) | PR #73, CI run 36260139965 job pr-check | — | PASS` — team-runner pack merged after pr-check SUCCESS (10m1s). `scripts/team-runner/` (provisioner + start template + REGISTER.md) with 8 invariant tests wired into `npm test`, mutant-proven (a weak guard assertion survived the first mutant and was strengthened). A2 awaits a team machine + human registration; no workflow change until the runner is online.
- `L-30 | 2026-09-26 | c6e18ce (merge of 2d9b43f) | PR #74, CI run 36261382481 job pr-check | — | PASS` — UI bundle budget guard + speed baseline merged after pr-check SUCCESS (10m15s). `scripts/check-ui-budget.mjs` runs at the end of `npm run build` (caps 420 kB raw / 130 kB gzip; RED-proven at 100 kB). Baseline: JS 350,816/105,791 B, CSS 43,222/8,882 B, build 2.66 s, node 311 in 11.7 s, Vitest 273. Packaged CDP timings are the next measure-before-optimize step.
